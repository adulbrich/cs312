...
2. **Check what services are listening on the network:**
   ...
   ```bash title="Check What Services Are Listening on the Network"
   sudo ss -tlnp
   ```
...
3. **Check the current SSH configuration:**
   ...
   ```bash title="Inspect SSH Configuration"
...
4. **List human login accounts:**
   ...
   ```bash title="List Human Login Accounts"
...
5. **Check recent login history and failures:**
   ...
   ```bash title="Check Recent Login History and Failures"
...
6. **Check the AppArmor profiles already in place:**
   ...
   ```bash title="Check the AppArmor Profiles Already in Place"
   sudo aa-status
   ```
...
1. **Verify your key-based login is working.** Before making any changes, open a second terminal and confirm you can SSH in again with your current key:
   ...
   ```bash title="Verify Your Key-based Login Is Working"
...
2. **Create a dedicated deploy user and grant it full passwordless sudo:**
   ...
   ```bash title="Create a Dedicated Deploy User and Grant It Full"
...
3. **Add your public key to the deploy user's authorized keys:**
   ...
   ```bash title="Add Your Public Key to the Deploy User's Authorized Keys"
...
4. **Test the deploy login from your second terminal:**
   ...
   ```bash title="Test the Deploy Login from Your Second Terminal"
...
5. **Harden `sshd_config`:**
   ...
   ```text title="Password Authentication Setting"
...
6. **Restart the SSH service and observe the lockout:**
   ...
   ```bash title="Restart the SSH Service and Observe the Lockout"
...
1. **Set the default policies:**
   ...
   ```bash title="Set the Default Policies"
...
2. **Allow SSH with rate limiting:**
   ...
   ```bash title="Allow SSH with Rate Limiting"
...
3. **Allow web traffic (add only the ports your server actually uses):**
   ...
   ```bash title="Allow HTTP and HTTPS"
...
4. **Enable the firewall and verify:**
   ...
   ```bash title="Enable the Firewall and Verify"
   sudo ufw enable
   ...
   ```text title="Part 3: Configure the Firewall Output"
   Status: active
   Logging: on (low)
...
5. **Confirm the port state from inside the server:**
   ...
   ```bash title="Confirm the Port State from Inside the Server"
   sudo ss -tlnp
   ```
...
4. **Verify the jail is active:**
   ...
   ```bash title="Verify the Jail Is Active"
   sudo fail2ban-client status sshd
   ```

   You should see output like:

   ```text title="Part 4: Install and Configure Fail2ban Output"
...
5. **Restore ubuntu SSH access now that Fail2Ban is active:**
   ...
   ```bash title="Restore Ubuntu SSH Access Now That Fail2ban Is Active"
   sudo vim /etc/ssh/sshd_config
   ```

   Change `AllowUsers deploy` to:

   ```text title="AllowUsers Setting"
...
1. **Replace the broad sudoers rule with a scoped one:**
   ...
   ```bash title="Replace the Broad Sudoers Rule with a Scoped One"
   ...
   ```text title="Scoped sudoers Rule"
...
2. **Verify the restriction works:**
   ...
   ```bash title="Verify the Restriction Works"
   sudo fail2ban-client status sshd
   ```
...
1. **Confirm the SSH configuration:**
   ...
   ```bash title="Verify Hardened SSH Settings"
   sudo sshd -T | grep -E "passwordauthentication|permitrootlogin|allowusers|pubkeyauthentication"
   ```
...
2. **Confirm the firewall rules:**
   ...
   ```bash title="Confirm the Firewall Rules"
   sudo ufw status verbose
   ```

3. **Confirm Fail2Ban is running:**

   ```bash title="Confirm Fail2ban Is Running"
   sudo fail2ban-client status sshd
   ```

4. **Check open ports one final time:**

   ```bash title="Check Open Ports One Final Time"
   sudo ss -tlnp
   ```
...
5. **Create a timestamped verification entry:**
   ...
   ```bash title="Create a Timestamped Verification Entry"
