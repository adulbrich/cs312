...
- Your SSH keypair (e.g., `cs312-key.pem`) that you used to launch the instance
...
6. **Check the AppArmor profiles already in place:**
   ...
   AppArmor is active by default on Ubuntu. The output lists profiles in two modes: **enforce mode** actually blocks operations the profile does not allow; **complain mode** only logs policy violations without blocking them, which is used when testing a new profile before committing to enforcement. On Ubuntu 26.04, `sshd` does not have its own AppArmor profile; Ubuntu removed it in favor of other kernel hardening mechanisms. You will see profiles for tools like `tcpdump`, `chronyd`, and `man` in enforce mode. The important observation is that AppArmor is active and enforcing profiles for many system utilities, and that its presence on this server means any application profile you later write will be enforced automatically.

</Steps>

<ActivityQuestion>
Looking at the output of `sshd -T`, what is the difference between `passwordauthentication no` (which AWS sets by default) and `permitrootlogin prohibit-password`? Each one closes a different attack vector: name each one specifically, and explain why having only one of the two still leaves a gap.
</ActivityQuestion>
...
6. **Restart sshd and observe the lockout:**
   ...
   The output should now show `allowusers deploy`. From your second terminal, try connecting as ubuntu:
...
5. **Confirm the port state from inside the server:**
   ...
   Compare this output to the list you made in Part 1. Every port that is now visible here should correspond to a UFW allow rule. If you see a port listening with no corresponding allow rule, UFW's default deny policy is dropping traffic to it from all sources.
...
4. **Verify the jail is active:**
   ...
   The counts start at zero. Check whether the ubuntu rejection from Part 2 appears in `Total failed`. AllowUsers rejections happen before authentication completes, so depending on the journal pattern Fail2Ban uses, it may or may not have counted it.
...
2. **Verify the restriction works:**
   ...
   You should see the Fail2Ban service status. Then test that a disallowed command fails:

   ```bash
   sudo apt update
   ```

   You should see `sudo: I'm sorry deploy. I'm afraid I can't do that`. The restriction is working. A compromised deploy account cannot install software, modify system files, or escalate further.
