...
6. Simulate upgrading the web service. Open `compose.yml` and change the `web` image from `nginx:alpine` to `nginx:latest`. Then pull the updated image and recreate the container:
   ...
   `docker compose pull` fetches `nginx:latest` and reports which images changed. `docker compose up -d` recreates only the containers whose image changed. The `echo` service is unchanged and left running.

   Verify the stack is still working:
   ...
   The HTML you put in `html/` is still served. The bind mount was never inside the old container; it survived the replacement unchanged.

7. Check which nginx version is actually running now:

   ```bash
   docker compose exec web nginx -v
   ```

   Compare it to what `nginx:alpine` would report. The container was replaced; the content was not.

8. Stop the stack and observe what persists:
...
<ActivityQuestion>
When you ran `docker compose up -d` after pulling `nginx:latest`, Compose recreated the `web` container but left `echo` running. What tells Compose which containers need to be replaced? If your application had written files inside the container's own filesystem (not in a volume or bind mount), would those files have survived the replacement?
</ActivityQuestion>
