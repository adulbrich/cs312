...
{/* ai-summary
type: activity
slug: docker-image-exploration
order: 5
paired_lecture: containerization-with-docker
practices: image layer inspection with docker history; process and filesystem namespace isolation; ephemeral storage loss on container removal; named volume persistence across restarts; multi-stage Dockerfile builds; port mapping; Docker Compose multi-service stack startup; container network inspection with docker network inspect
prereq_activities:
output: a running multi-service Compose stack with a custom-built image and a named volume that survives container restarts
*/}
...
**Rancher Desktop**: Download from [rancherdesktop.io](https://rancherdesktop.io/). Free and open-source; bundles either `containerd` or `dockerd`.

Verify your installation:

```bash title="Rancher Desktop"
docker version
...
**Rancher Desktop**: Download from [rancherdesktop.io](https://rancherdesktop.io/). Provides a GUI and bundles either `containerd` or `dockerd`.
...
2. Pull two variants of the nginx image:
   ...
   ```bash title="Docker Pull Nginx:alpine"
...
3. Compare their sizes:
   ...
   ```bash title="Docker Images Nginx"
   docker images nginx
   ```
...
4. Inspect the layers of the alpine image:
   ...
   ```bash title="Docker History Nginx:alpine"
...
5. Examine the image's metadata:
   ...
   ```bash title="Docker Inspect Nginx:alpine"
...
1. Start an nginx container in the background and then open an interactive shell inside it:
   ...
   ```bash title="Start nginx Sandbox"
...
3. Exit the container and run the same commands on your host machine:
   ...
   ```bash title="Exit Container Shell"
   exit
   hostname
   whoami
   ps aux
...
1. The `sandbox` container from the previous section is still running. Generate an HTTP request inside it to produce a log entry, then view the logs from the host:
   ...
   ```bash title="Generate and View nginx Logs"
...
2. Check resource usage across all running containers:
   ...
   ```bash title="Docker Stats --no-stream"
...
3. Stop the container and observe its status:
   ...
   ```bash title="Docker Stop Sandbox"
   docker stop sandbox
   docker ps
   docker ps -a
   ```
...
4. Restart it:
   ...
   ```bash title="Docker Start Sandbox"
...
2. Inspect what changed in the container's writable layer:
   ...
   ```bash title="Docker Diff Ephemeral"
...
3. Stop and remove the container completely:
   ...
   ```bash title="Docker Stop Ephemeral"
...
5. Clean up the second container:
   ...
   ```bash title="Docker Stop Ephemeral2"
...
1. Create a named volume:
   ...
   ```bash title="Docker Volume Create Mydata"
...
4. Find where Docker stores the volume on your host:
   ...
   ```bash title="Docker Volume Inspect Mydata"
...
2. Inspect the container's network configuration:
   ...
   ```bash title="Docker Inspect Webserver"
...
5. Create a user-defined network and run two containers on it:
   ...
   ```bash title="Docker Network Create Mynet"
...
6. Confirm that containers on different networks cannot reach each other:
   ...
   ```bash title="Confirm That Containers on Different Networks Cannot Reach"
...
7. Clean up:
   ...
   ```bash title="Docker Stop Webserver Server1 Server2"
...
6. Confirm that the `web` container can resolve the `cache` hostname via Compose's embedded DNS:
   ...
   ```bash title="Resolve Compose Service Name"
...
7. Upgrade the web service. Open `compose.yml` and change the `web` image from `nginx:alpine` to `nginx:latest`, then pull and recreate:
   ...
   ```text title="A Multi-service Stack with Compose Output"
...
5. Verify the container process is running as a non-root user:
   ...
   ```bash title="Docker Exec Myapp Whoami"
...
6. Tag the image in the format a registry expects. Replace `YOURONID` with your actual ONID:
   ...
   ```bash title="Docker Tag Myapp:v1 Youronid/myapp:v1"
...
7. Inspect the image layers:
   ...
   ```bash title="Docker History Myapp:v1"
