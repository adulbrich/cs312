...
This activity puts into practice the concepts from the Containerization with Docker lecture. Instead of following a tutorial, you will investigate how containers actually behave: what isolation they provide, what they do and do not preserve, and how the tools you use every day translate into kernel-level operations.
...
<Steps>
1. Pull two variants of the nginx image:
...
3. Inspect the layers of the alpine image:
...
4. Examine the image's metadata:
...
<ActivityQuestion>
`docker history` shows layers oldest-first (at the bottom) or newest-first (at the top). Looking at the output, which layer represents the base OS, and which layers represent nginx-specific installation steps? How does this reflect the order of instructions in a Dockerfile?
</ActivityQuestion>
...
<ActivityQuestion>
Inside the container, `ps aux` showed only a handful of processes, and the nginx process appeared as PID 1 (or close to it). On your host, `ps aux` shows many more. Both are running on the same physical machine with the same kernel. What Linux feature explains why the container sees a different, smaller set of process IDs?
</ActivityQuestion>
...
<ActivityQuestion>
You modified the running container's filesystem in step 4, but the base image was unchanged. Where was your modification stored, and why did it disappear when you removed the container?
</ActivityQuestion>
