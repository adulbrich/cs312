...
**Docker Desktop**: Download from [docker.com/products/docker-desktop](https://www.docker.com/products/docker-desktop/). Docker Desktop will offer to enable WSL2 during installation if it is not already configured. After installation, open a WSL2 terminal (e.g., Ubuntu from the Start menu) and verify:
...
5. Create a user-defined network and run two containers on it:
   ...
   `wget` downloads a URL from the command line. `-q` suppresses progress output; `-O-` writes the response to standard output instead of saving it to a file. Container DNS resolves `server1` to its IP within `mynet`.
...
1. Create a working directory and an HTML file with your ONID:
   ...
   echo "<h1>Built by: YOUR_ONID</h1>" > index.html
   ```
