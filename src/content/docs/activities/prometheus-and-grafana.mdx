...
Before the first query, keep this PromQL syntax refresher nearby:

- `metric_name{label="value"}` selects time series by metric name and exact label match.
- `label=~"regex"` uses a regex label match instead of exact match.
- `[5m]` after a metric selects a 5 minute range vector.
- `rate(counter[5m])` converts a counter to a per-second rate over that window.
- `sum by (label1, label2) (...)` aggregates series while keeping only listed labels.
- `histogram_quantile(0.95, ...)` computes p95 from histogram bucket series.
