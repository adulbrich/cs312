...
slug: vm-setup
order: 2
paired_lecture: virtualization-basics
practices: Type-2 hypervisor installation and VM configuration; CPU, RAM, and disk allocation tradeoffs; VirtualBox guest additions for paravirtualized drivers; NAT vs host-only networking modes; snapshot creation and restore workflow; baseline CPU, memory, and disk metric collection
prereq_activities:
output: a running Debian VM with host-only networking, SSH access configured, and at least one snapshot created and restored
*/}
...
1. Check which storage devices the kernel sees:
   ...
   ```text title="Verify Paravirtualized Drivers Output"
   NAME TYPE TRAN
   sda  disk
   sr0  rom
   ```
...
2. Confirm the VirtIO controller is active by checking the PCI device list:
   ...
   ```bash title="Lspci | Grep -i Virtio"
   lspci | grep -i virtio
   ```
...
3. Install `ethtool` to query the network driver. On Debian, `sudo` is not installed by default, so switch to root first:
   ...
   ```bash title="Become Root to Install Tools"
   su -
   apt update
   apt install ethtool
   exit
...
5. Check which driver is managing the network interface (substitute your interface name):
   ...
   ```bash title="Inspect Network Driver"
...
1. Sample the CPU idle percentage:
   ...
   ```bash title="Sample CPU Idle Percentage"
...
1. Append a bad config line and reload the service:
   ...
   ```bash title="Break SSH Configuration"
   su -
...
1. Look at the end of the SSH config:
   ...
   ```bash title="Tail -10 /etc/ssh/sshdconfig"
...
3. Confirm the VM still has outbound internet access; run this **inside the VM**:
   ...
   ```bash title="Ping -c 3 Debian.org"
   ping -c 3 debian.org
   ```
...
1. Install cowsay and fortune:
   ...
   ```bash title="Install cowsay and fortune"
   su -
