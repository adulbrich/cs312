...
7. **Calculate quantity.** How many of this server SKU do you need to meet the scenario's demand? Show your math.
   - **Scenario A:** Enough servers to host 200 VMs with the stated memory requirement. You may assume a moderate 2:1 vCPU-to-physical-core consolidation ratio if you state it explicitly. Here, 2:1 means planning that two virtual CPUs can share one physical CPU core on average because not every VM is busy at the same moment. Do not overcommit memory. Leave at least one whole host worth of headroom so a single failure does not break the scenario.
...
<Steps>
1. **Count access ports needed.** Each server typically needs two data Network Interface Cards (NICs) for redundancy plus one BMC NIC. Total the data ports first.
...
4. **Add your name and student ID** to the header of the document.

5. **Save the completed document.** Use a clear filename. If you want a one-page handout, export it to PDF after tightening the layout. Then read it top to bottom once more and confirm every number has either a source or a short calculation next to it.
