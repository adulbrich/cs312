...
- `auditd` on the Linux machine. Install it if it is not already present: `sudo apt-get install -y auditd && sudo systemctl enable auditd --now`
...
4. **Correlate the degradation window across application, database, and host logs:**
    ...
    ```bash
    grep 'database connection timeout\|connection pool exhausted' ~/cs312-log-activity/bundle/order-api.jsonl
    grep 'remaining connection slots\|logical replication launcher' ~/cs312-log-activity/bundle/postgres.log
    grep 'nightly-export.service still running\|holding open sessions' ~/cs312-log-activity/bundle/db-host-journal.log
    ```

    These commands surface the supporting entries around the failure window without assuming exact timestamps up front. `order-api.jsonl` shows two early database timeouts that still served stale cache, then a pool-exhaustion warning at `03:03:14`, and finally a timeout at `03:03:58` with no fallback left. `postgres.log` shows connection-slot exhaustion at `03:01:04` and a later worker issue at `03:04:13`. `db-host-journal.log` shows the export service still running at `03:01:05` and still holding sessions at `03:04:14`.
