...
description: "Investigate a staged incident using journalctl and structured log files, trace a container crash with kubectl, deploy Fluent Bit as a DaemonSet to see log enrichment in action, verify a file access event with auditd, and reconstruct a cause chain as an incident timeline."
sidebar:
  order: 14
draft: true
---
...
practices: journalctl filtering by tag, priority, and boot; scoping an investigation from edge logs to application and database sources; kubectl logs --previous and kubectl describe events for container crash investigation; Fluent Bit DaemonSet deployment and Kubernetes metadata enrichment; correlation ID tracing across multiple log sources; auditd watch rules and ausearch for kernel-level audit events; incident timeline table construction from four sources
prereq_activities: vm-setup, minikube
output: an incident timeline file for the staged server incident plus a live Fluent Bit stream showing Kubernetes metadata enrichment
*/}
...
- `journalctl`, `logger`, `grep`, and `awk` on that server (all present by default on Ubuntu 22.04)
- `auditd` on the server. Install it if it is not already present: `sudo apt-get install -y auditd && sudo systemctl enable auditd --now`
...
Before investigating anything, you need a controlled set of logs to work with. Every student investigates the same failure sequence: a database connection pool exhausted by a long-running export job, causing an order API to return HTTP 500 errors. Three of the files in this bundle simulate the flat log files you would usually find under `/var/log/` or an application-specific log directory, while `db-host-journal.log` is a saved excerpt from another machine's systemd journal. The journal entries you write here are real: `logger` sends them to the same local logging path that many Linux services use automatically through systemd-managed stdout and stderr or the syslog socket at `/dev/log`.
...
3. **Find the earliest application-side error:**
    ...
    The first application error appears at `03:01:17`, almost three minutes before the first HTTP 500. That gap is the MTTD window if alerting fires only on HTTP error rate. Use this earlier application error as the anchor for the deeper investigation.

4. **Correlate the degradation window across application, database, and host logs:**

    ```bash
    grep '"request_id":"req-1002"' ~/cs312-log-activity/bundle/order-api.jsonl
    grep '03:01:04\|03:04:13' ~/cs312-log-activity/bundle/postgres.log
    grep '03:01:05\|03:04:14' ~/cs312-log-activity/bundle/db-host-journal.log
    ```

    These commands surface the exact supporting entries around the failure window. `order-api.jsonl` shows the first timeout at `03:01:17`. `postgres.log` shows connection-slot exhaustion at `03:01:04` and a later worker issue at `03:04:13`. `db-host-journal.log` shows the export service still running at `03:01:05` and still holding sessions at `03:04:14`.

5. **Widen the window backward to find what started the chain:**

    ```bash
    grep 'Started nightly-export.service\|opened 90 database sessions' ~/cs312-log-activity/bundle/db-host-journal.log
    ```

    This surfaces the precursor before any user-visible failure: the export job started at `02:58:23` and opened 90 database sessions at `02:58:24`. That is the earliest concrete evidence explaining why the later timeouts happen.
...
2. **Create the timeline file from the evidence you just surfaced:**
    ...
    exhausting the connection pool and leading to order-api timeouts.
...
3. **Print the timeline:**
    ...
    Your name, hostname, and cause chain should all appear in one clean block. Notice the gap between internal failure (`03:01:04`) and first user-visible symptom (`03:04:01`): almost three minutes during which the application was degrading before requests started failing. That gap is the MTTD window if alerting fires only when the HTTP 5xx rate rises.
...
<Aside type="note" title="Rotation handles size; agents handle ephemerality">
`logrotate` keeps disk usage bounded on a long-running server. It does not help when a pod is deleted: container log files in `/var/log/pods/` are removed with the pod. Fluent Bit, running as a DaemonSet, ships logs off the node before the pod disappears. That is the gap that a collection agent closes, and the reason the lecture distinguishes between local rotation and centralized collection.
</Aside>
