...
order: 7
paired_lecture: infrastructure-as-code
practices: provider and data source blocks; resource blocks for security group and EC2 instance; outputs and input variables; plan/apply/destroy workflow; state file inspection with terraform state list and terraform show; terraform import of an existing resource; HCL refactoring with locals
prereq_activities: aws-console-tour
output: a Terraform configuration that provisions a security group and EC2 instance, with the VPC from the previous activity imported into state, followed by a complete destroy
*/}
...
1. **Verify the AWS CLI is installed:**
   ...
   ```bash title="Verify the AWS CLI Is Installed"
   aws --version
   ```
...
4. **Store the credentials** using the method that suits your environment:
   ...
   ```ini title="AWS Credentials Profile"
   [default]
...
5. **Verify the credentials work** before Terraform touches anything:
   ...
   ```bash title="Verify the Credentials Work"
...
1. **Create a project directory:**
   ...
   ```bash title="Create a Project Directory"
...
2. **Write `main.tf`:**
   ...
   `owners = ["099720109477"]` limits results to images [published by Canonical](https://documentation.ubuntu.com/aws/aws-how-to/instances/find-ubuntu-images/). Ubuntu 26.04 images uses the `hvm-ssd-gp3` naming family, so an older `hvm-ssd` filter will not match current releases.
...
6. **Validate the configuration:**
   ...
   ```text title="Initialize the Project Output"
   Success! The configuration is valid.
   ```

7. **Read the first plan:**

   ```bash title="Read the First Plan"
   terraform plan
   ```
...
2. **Wire the security group to the instance** by adding this line inside `resource "aws_instance" "web"`:
   ...
   ```hcl title="Wire the Security Group to the Instance"
...
4. **Find the `vpc_security_group_ids` line** in the instance section of the plan:
   ...
   ```text title="Find the Vpcsecuritygroupids Line Output"
...
4. **Read one output by name:**
   ...
   ```bash title="Read Terraform Output Value"
...
3. **Update both resource blocks** to use `var.instance_type` and `local.common_tags`:
   ...
   ```hcl title="Update Both Resource Blocks"
   resource "aws_security_group" "web" {
     name        = "tf-activity-web-sg-${var.onid}"
     description = "Activity web security group"
     vpc_id      = data.aws_vpc.default.id
...
4. **Format and validate:**
   ...
   ```bash title="Format and Validate"
   terraform fmt
   terraform validate
   ```
...
2. **Write `modules/db_sg/variables.tf`:**
   ...
   ```hcl title="modules/db_sg/variables.tf"
   variable "onid" {
...
3. **Write `modules/db_sg/main.tf`:**
   ...
   ```hcl title="modules/db_sg/main.tf"
   data "aws_vpc" "default" {
     default = true
   }
...
4. **Write `modules/db_sg/outputs.tf`:**
   ...
   ```hcl title="modules/db_sg/outputs.tf"
   output "security_group_id" {
...
5. **Call the module from root `main.tf`** by adding this block below the `locals` block:
   ...
   ```hcl title="Call the Module from Root Main.tf"
   module "db_sg" {
...
7. **Re-initialize to register the new module:**
   ...
   ```bash title="Re-initialize to Register the New Module"
   terraform init
   ```

   You should see:

   ```text title="Create a Reusable Module Output"
...
9. **Apply and inspect:**
   ...
   ```bash title="Apply and Inspect"
   terraform apply
   ```

   After applying, run:

   ```bash title="Inspect Terraform State"
...
1. **Find the `cs312-vpc` ID:**
   ...
   ```bash title="Find the Cs312-vpc ID"
...
3. **Import the existing VPC into Terraform state:**
   ...
   ```bash title="Import the Existing VPC into Terraform State"
   terraform import aws_vpc.cs312 <VPC_ID>
   ```

   Replace `<VPC_ID>` with the value from step 1. You should see:

   ```text title="Import an Existing Resource Output"
   Import successful!
...
5. **Inspect the recorded state** to see the full attribute set:
   ...
   ```bash title="Inspect the Recorded State"
...
7. **Remove the imported VPC from this configuration** before proceeding to the destroy step:
   ...
   ```bash title="Remove Imported VPC from State"
...
2. **Run the plan:**
   ...
   ```text title="Plan a Real Change, Inspect State, and Clean Up Output"
   ~ instance_type = "t3.micro" -> "t3.small"
   ```

3. **Compare the plan with the current state:**

   ```bash title="Compare the Plan with the Current State"
...
4. **List all managed resources:**
   ...
   ```bash title="List All Managed Resources"
   terraform state list
   ```
...
6. **Confirm the state is empty:**
   ...
   ```bash title="Confirm the State Is Empty"
   terraform state list
   ```
...
```hcl title="Terraform Lifecycle Block"
lifecycle {
