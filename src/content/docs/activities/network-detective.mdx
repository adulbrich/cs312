...
{/* ai-summary
type: activity
slug: network-detective
order: 4
paired_lecture: networking-fundamentals
practices: interface enumeration with ip addr and ip route; ARP cache inspection; traceroute path analysis across multiple NAT hops; DNS resolution with dig (A, MX, PTR records); open port scanning with nmap; TCP connection state inspection with ss
prereq_activities: arch-linux-install
output: a completed record of the VM's interface configuration, routing table, ARP cache, DNS resolution results, and port scan findings from both inside and outside the VM
*/}
...
<Aside type="tip" title="Compare Host and VM Views">
...
5. Inspect the layer-2 neighbor (ARP) table:
   ...
   ```text title="Layer 1-3: Link, Address, and Neighbors Output"
...
2. Inspect the DHCP lease details for your interface:
   ...
   ```bash title="Inspect DHCP Lease State"
...
3. Now compare that DNS server to what your system actually uses for name resolution:
   ...
   ```bash title="Inspect Resolver Configuration"
   cat /etc/resolv.conf
   ```
...
2. Can you reach the gateway?
   ...
   ```bash title="Interface Used for the Default Route"
...
4. Does DNS work?
   ...
   ```bash title="Ping -c 4 Google.com"
   ping -c 4 google.com
   ```
...
2. Try TCP traceroute:
   ...
   ```bash title="Trace TCP Path to the Internet"
...
3. Run traceroute from your **host machine** where it works correctly:
   ...
   ```bash title="On Your Host Terminal (Not Inside the VM)"
...
4. Compare a nearby host to a distant one, from the host machine:
   ...
   ```bash title="Traceroute Oregonstate.edu"
...
3. Find sshd. You should see:
   ...
   ```text title="What Is Running on This Machine? Output"
...
4. Now scan this machine from the outside. On your **host machine** (not inside the VM), open a terminal and run:
   ...
   ```bash title="Nmap -sv -pn -p T:2222 127.0.0.1"
...
5. Scan the VM's localhost from inside:
   ...
   ```bash title="Nmap -sv -pn -p T:1-1024 127.0.0.1"
...
4. Explore CNAME (alias) records:
   ...
   ```text title="DNS Investigation Output"
...
```bash title="Summarize Network Timings"
printf "Hostname: %s\nLocal IP: %s\nGateway:  %s\nPublic IP: %s\nDNS:      %s\n\n" \
  "$(cat /etc/hostname)" \
