...
- Unless a step explicitly says **host machine**, run commands in the **VM terminal** (your SSH session to `archuser@127.0.0.1 -p 2222`).
...
1. Check whether the network interface has a link:
   ...
   Every interface has a state: `UP` means the hardware link is active; `DOWN` means no physical or virtual link was detected. Look for your main interface (likely `enp0s3` in VirtualBox) and confirm it shows `state UP`. If it shows `DOWN`, no higher-layer tools will work: the cable is disconnected, the virtual NIC is unconfigured, or the interface was manually brought down.
...
4. Does DNS work?
   ...
   If the previous step worked but this fails, there might be several possible causes. The most likely is that the IP address returned by our Cloudflare resolver is one where Google is blocking ICMP traffic, which is common. We'll explore that later. 

   If this fails, try a different hostname (e.g., `oregonstate.edu`) to rule out a Google-specific issue. If all hostnames fail, DNS is likely broken.
</Steps>
...
2. Check UDP listening sockets:
   ...
   This contrast is intentional: most application protocols (SSH, HTTP, databases) run over TCP because they need the reliability guarantees. DNS and DHCP use UDP because they favor speed and simplicity; a single small request and reply, with retries handled in the application layer rather than the transport layer.
...
4. Inspect the TLS handshake directly with `openssl s_client`:
   ...
   ```bash
   openssl s_client -connect oregonstate.edu:443
   ```
   ...
   If the verify return code is anything other than `0 (ok)`, you have a TLS problem. Common non-zero codes: `10` means the certificate has expired, `20` means a certificate in the chain could not be found, `62` means the hostname does not match the certificate. These are the same errors `curl` would report as a failed TLS handshake, but `openssl s_client` shows you exactly where in the chain the failure occurred.
...
1. Print your machine's identity and address information:
   ...
   Your hostname is the personal identifier in this output: it is the name you set during the Arch Linux install and it appears in your terminal prompt as well. You should see your local address in the `10.0.2.x` range, a different public IP (confirming NAT is working), and the DNS resolver you configured manually.
