...
With nginx installed, configure it and deploy a page that identifies the server. This section uses `sed` to modify the nginx configuration file and a [heredoc](https://en.wikipedia.org/wiki/Here_document) to write the HTML, and because the page includes a deployment timestamp and the restart is unconditional, rerunning it will update the page and restart nginx again.
