...
slug: scripting
order: 8
paired_lecture: shell-scripting-and-automation-basics
practices: shebang line and exit code conventions; variable assignment and command substitution; conditional logic with if/else; apt package installation from a script; systemd service status check with systemctl; cron job registration in /etc/cron.d; idempotent script design with existence checks
prereq_activities: aws-console-tour
output: a Bash script that installs nginx, deploys a custom page containing the student's ONID, and registers a cron-based health check on an EC2 instance
*/}

This activity puts into practice the concepts from the [Shell Scripting and Automation Basics](/lectures/shell-scripting-and-automation-basics/) lecture. You will write a Bash script that installs nginx on your EC2 instance, deploys a custom page with your ONID, and adds a cron-based health check. By the end, you will have a working imperative setup script you can rerun predictably and compare against the declarative Ansible workflow in the next activity.
...
2. Print your interactive shell's PATH:
   ...
   ```text title="Connect and Explore the Environment Output"
...
3. Simulate a minimal cron-like environment:

   ```bash title="Show Environment Variables"
...
<ActivityQuestion>
You simulated a minimal cron-like PATH of `/usr/bin:/bin`. After installing nginx in a later section, run `which nginx`. Is nginx in `/usr/bin` or `/usr/sbin`? Is `/usr/sbin` in that minimal PATH? What practical rule avoids this entire class of failure when writing scripts that cron will run?
</ActivityQuestion>
...
2. Open `setup.sh` in any editor and add a shebang and a single print statement:
   ...
   ```text title="Your First Script Output"
...
4. Add `set -euo pipefail` immediately after the shebang line. The file now starts:
   ...
   ```bash title="Compare unset Variable Behavior"
...
6. Run the script correctly with your actual ONID:
   ...
   ```bash title="Run Script with Valid Input"
...
2. Add a disk check below the `log` function:
   ...
   ```bash title="Calculate Root Disk Usage"
...
3. Run the script to test the disk check:
   ...
   ```text title="Check Preconditions Output"
   === Server Setup: your-onid on ip-10-0-1-42 ===
...
5. Test `is_installed` directly in the shell before relying on it in the script:
   ...
   ```bash title="Package Installed Helper"
...
6. Run `grep -E` directly against a package that is already installed so you can see the status codes the pattern is matching:
   ...
   ```bash title="Inspect dpkg Status Codes"
...
2. Add a package list and loop below the disk check block:
   ...
   ```bash title="Add Package Installation Logging"
...
3. Run the script:
   ...
   ```text title="Install Packages Output"
...
With nginx installed, configure it and deploy a page that identifies the server. This section uses `sed` to update the `server_name` directive in the nginx configuration, and a [heredoc](https://en.wikipedia.org/wiki/Here_document) to write the HTML. Because the page includes a deployment timestamp and the restart is unconditional, rerunning the script will update the page and restart nginx again.
...
1. Confirm that the default nginx configuration is valid before modifying it:
   ...
   ```text title="Configure Nginx and Deploy Your Page Output"
...
3. Add the index page right after the `sed` line:
   ...
   ```bash title="Write nginx Index Page"
...
4. Add the config test, restart, and verification right after the heredoc:
   ...
   ```bash title="Validate nginx Configuration in Script"
   nginx -t 2>>"$LOG_FILE"
...
<Steps>
1. Confirm that cron is present and active:
   ...
   ```bash title="Apt-get Install -y Cron"
...
2. Add the health-check installation block to `setup.sh`, below the nginx verification:

   ```bash title="Create Health Check Script"
...
3. Add the cron job registration right after:

   ```bash title="Schedule Health Check Cron Job"
...
4. Add a final completion line at the very end of `setup.sh`:
...
7. Wait about 60 seconds, then check the health log:
   ...
   ```text title="Schedule a Health Check Output"
...
3. From your own machine, verify the page over the public internet:
   ...
   ```html title="Your Configured Server HTML"
