...
This activity puts into practice the concepts from the [Configuration Management with Ansible](/lectures/configuration-management/) lecture. Where the earlier scripting activity handled the same web-server setup imperatively from inside the EC2 instance, here you will reimplement it declaratively with Ansible from your control machine over SSH. By the end, nginx will be installed, running, serving your custom page, and monitored by the same cron health check, all through a playbook you can re-run safely and commit to version control.
...
2. **Set the instance name and choose an AMI.**
   ...
   - Name: `cs312-ansible-lab`
   - AMI: Ubuntu Server 24.04 LTS (the default Ubuntu option)
...
4. **Configure the security group.**
   ...
   Under "Network settings," click "Edit." Create a new security group named `cs312-ansible-sg` with two inbound rules:
...
1. **Install ansible-lint:**
    ...
    ```
    macOS users who installed Ansible with Homebrew can also run `brew install ansible-lint`.

2. **Create the first version of `site.yml`:**
    ```yaml
    ---
...
3. **Run the linter against this first version:**
    ```bash
    ansible-lint site.yml
    ```
    A clean playbook produces no output. If you see warnings, fix them before continuing.

4. **Preview what this version will do:**
    ```bash
    ansible-playbook -i hosts.ini site.yml --check --diff
    ```
    This preview should predict package and service changes. There may not be much diff output yet because this version does not manage any file content.
...
6. **Run the same playbook a second time right away:**
    ```bash
    ansible-playbook -i hosts.ini site.yml
    ```
    On this immediate second run, both tasks should now show `ok`. The server is already in the declared state.

7. **Verify that nginx is serving its default page:**
    ```bash
...
You have built the core Ansible workflow: inventory, ad-hoc exploration to understand a system, a playbook with handlers that is safe to re-run, lint checks before every run, and a dry-run preview before applying changes. The natural next step is to make playbooks more flexible and reusable.

A small but impactful structural improvement is to move `student_name` out of `site.yml` and into a `group_vars/managed.yml` file alongside `hosts.ini`. Once you do that, the playbook itself contains no environment-specific values and can target any group with the right variable files in place. The [Variables guide](https://docs.ansible.com/ansible/latest/playbook_guide/playbooks_variables.html) explains `group_vars` and `host_vars` in detail.
