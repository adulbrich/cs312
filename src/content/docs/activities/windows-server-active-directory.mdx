...
prereq_activities:
output: a working Active Directory domain with two EC2 instances, at least two OUs, multiple users, and a GPO with one policy applied and confirmed with gpresult /scope computer /r
*/}
...
- A laptop with a Remote Desktop client installed:
  ...
  - **macOS:** [Windows App](https://apps.apple.com/us/app/windows-app/id1295203466?mt=12) from the App Store
...
<Steps>
1. In the AWS Console, navigate to **VPC** dashboard.
...
<Aside type="note" title="Why two Availability Zones?">
In a production environment, you would place domain controllers in separate AZs so that a single data center failure does not take down authentication for the entire domain. We replicate that pattern here even though we are only deploying one DC.
</Aside>
...
2. Configure the instance:
   ...
   - **Key pair:** Use your existing key pair or create a new one (you will need this to retrieve the Administrator password and connect via RDP)

3. Under **Network settings**, click **Edit**:
   - **VPC:** Select `cs312-ad-vpc`
   - **Subnet:** Select the one ending in `a`
...
<Aside type="caution" title="Be Patient After Promotion">
...
2. Network settings:
   - **VPC:** `cs312-ad-vpc`
   - **Subnet:** Select the one ending in `b` (a different AZ from the DC)
   - **Auto-assign public IP:** Enable
...
<Aside type="note" title="Why Windows Server for a Client?">
You might wonder why we use Windows Server as a "client" instead of Windows 11. AWS only allows server OS images on EC2. For our purposes, this does not matter: domain joining, Group Policy, and user authentication work identically on both server and desktop editions.
</Aside>
...
<Steps>
1. Open **Advanced network settings** (search "network" in the Start menu).

2. Click **Edit** under "More adapter options" under the Ethernet adapter.
...
7. Right-click the new `student` user and select **Add to a group**. Type `Domain Admins` and click OK.
   ...
   <Aside type="caution" title="Domain Admins Warning">
   Adding a user to Domain Admins is done here for simplicity. In production, you would never grant Domain Admin to regular users.
   </Aside>
</Steps>
...
1. Connect to the **WindowsClient** via RDP. This time, log in with the domain account:
   ...
   <Aside type="tip" title="Domain Login Tip">
...
3. Open a **Command Prompt** and verify your identity:
   ...
   ```text title="Check Domain Login Identity"
   whoami
   ```
...
4. Force Group Policy to apply immediately (rather than waiting up to 90 minutes for the automatic refresh):
   ...
   ```text title="Force Group Policy Update"
   gpupdate /force
   ```

5. Verify the GPO took effect by checking the PATH:

   ```text title="Check Updated PATH"
   echo %PATH%
   ```

   You should see `C:\Users\Public\cs312` in the output. Restart the command prompt for the change to take effect if you don't see it immediately.

6. For a detailed report of which computer-side GPOs are applied to this machine, open **Command Prompt as administrator** and run:

   ```text title="Generate Group Policy Report"
   gpresult /scope computer /r
   ```

   Look for `Lab Policy` under "Applied Group Policy Objects."
</Steps>
...
2. In the **VPC Dashboard**, delete the `cs312-ad-vpc` VPC (this also removes its subnets, route tables, and internet gateway).
