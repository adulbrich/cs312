...
5. **Retrieve the Administrator password.** It takes a few minutes after launch before the password is available. Select the instance, click **Connect**, choose the **RDP client** tab, and click **Get password**. Upload your `.pem` key file and decrypt.
   ...
   <Aside type="note">
   **Lecture connection:** The lecture describes DSRM passwords and secure credential storage. AWS uses a similar principle here: the password is encrypted with your SSH key pair so that it is never transmitted or stored in plaintext. Only someone with the private key can retrieve it.
   </Aside>
...
5. In the promotion wizard:
   ...
   <Aside type="note">
   **Lecture connection:** The lecture explains that the first domain you create automatically becomes the root of a new forest. The domain name `cs312lab.local` becomes both the domain and the forest root. The `.local` TLD is conventional for internal domains that are not publicly resolvable.
   </Aside>
...
<Aside type="note">
**Lecture connection:** Behind the scenes, the domain join process created a computer object in Active Directory's default "Computers" container, registered DNS records, and established a trust relationship with the domain. This is the same process the lecture describes in the *Promoting a Domain Controller* section.
</Aside>
...
3. Right-click `cs312lab.local` and select **New Organizational Unit**. Name it `Lab Computers`. Click OK.
   ...
   <Aside type="note">
   **Lecture connection:** The lecture explains that OUs serve two purposes: organizing objects and serving as targets for Group Policy. You are creating this OU specifically so you can link a GPO to it in the next section.
   </Aside>
...
3. **Disable local GPO overrides.** Navigate to:
   ...
   <Aside type="note">
   **Lecture connection:** The lecture explains LSDOU processing order (Local, Site, Domain, OU). By disabling local GPO processing, you ensure that only domain-level and OU-level policies take effect, preventing users from setting local policies that override your centralized configuration.
   </Aside>
