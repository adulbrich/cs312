...
3. Update the package database and install the partitioning tool:
   ...
   `-S` tells pacman to install a package; `-y` refreshes the local copy of the package database from the mirrors before installing. The `gptfdisk` package provides the `gdisk` command, which is not included in the live environment by default.
...
2. **Create the EFI System Partition.** Type `n` for a new partition. Accept the default partition number (1) and default first sector. For the last sector, type `+300M`. When prompted for the hex code, enter `ef00` (EFI System Partition).
   ...
   The `+300M` notation means "300 megabytes from the current position" -- gdisk calculates the exact sector number for you. The hex code `ef00` sets the partition type flag that tells the firmware "this partition contains bootloader files."
...
Partitioning only defines boundaries on the disk. Formatting writes the actual filesystem data structures -- the inode tables, directory trees, and free-space maps -- that the kernel needs to store files. Each partition gets the filesystem appropriate to its role.
...
2. Open the sudoers file (never edit it directly, always use `visudo`):
   ...
   `visudo` opens `/etc/sudoers` in a safe wrapper that validates the syntax before saving. A syntax error in sudoers can lock you out of sudo entirely, so direct editing with `vim` is dangerous.
...
7. Configure DNS so you can resolve domain names:
   ...
   `/etc/resolv.conf` tells the system which DNS server to send name queries to. `8.8.8.8` and `8.8.4.4` are Google's public resolvers. The system tries the first nameserver and falls back to the second if it does not respond. On production systems this file is usually managed automatically by systemd-resolved or NetworkManager; here we write it manually so nothing is hidden.
