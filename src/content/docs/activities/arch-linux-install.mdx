...
order: 3
paired_lecture: linux-server-planning-and-configuration
practices: disk partitioning with fdisk; EFI System Partition and swap creation; ext4 filesystem formatting; arch-chroot pivot for installed-system configuration; locale, timezone, and hostname setup; user creation and sudo access; GRUB bootloader installation; network interface configuration; SSH server setup
prereq_activities: vm-setup
output: a minimal Arch Linux VM that boots independently, has a non-root user account, and accepts SSH connections on port 2222
*/}
...
2. Verify you have an internet connection (the VM's NAT adapter piggybacks on your host's network):
   ...
   ```bash title="Ping -c 3 Archlinux.org"
...
2. **Create the EFI System Partition.** Type `n` for a new partition, then `p` for primary partition. Accept the default partition number (1) and default first sector. For the last sector, type `+300M`. After that type `t`, select partition 1, and set the hex code to `uefi` or `ef` (EFI System Partition).
   ...
   ```text title="Create the EFI System Partition Output"
   ...
   ```text title="Partition the Disk Output"
...
5. **Verify your work.** Type `p` to print the partition table. You should see something like:
   ...
   ```text title="Verify Your Work Output"
...
1. Format the EFI partition as FAT32:
   ...
   ```bash title="Write the Partition Table"
...
3. Initialize and activate swap:
   ...
   ```bash title="Enable Swap and Mount Partitions"
   mkswap /dev/<disk>2
   swapon /dev/<disk>2
   ```
...
<Aside type="note" title="Verify Mounted Filesystems">
...
1. Install the base packages:
   ...
   ```bash title="Install Arch Base System"
...
2. Generate the filesystem table:
   ...
   ```bash title="Genfstab -U /mnt /mnt/etc/fstab"
...
1. Set the timezone (adjust if you are not on the US West Coast):
   ...
   <Aside type="tip" title="Sync the Hardware Clock">
   Sync the hardware clock so the RTC and system clock match:

   ```bash title="Hwclock --systohc"
   hwclock --systohc
   ```
   </Aside>
...
2. Configure the locale. Open the locale configuration file:
   ...
   ```bash title="Edit /etc/locale.gen"
   vim /etc/locale.gen
   ```

   Find and uncomment these two lines (remove the leading `#`):

   ```text title="locale.gen Entry"
   en_US.UTF-8 UTF-8
   en_US ISO-8859-1
   ```
...
1. Install GRUB and supporting EFI tools:
   ...
   ```bash title="Install GRUB and Supporting EFI Tools"
...
2. Install GRUB to the EFI partition:
   ...
   <Tabs syncKey="arch">
   <TabItem label="x86_64 (Intel/AMD)">
   ```bash title="X8664 (Intel/amd)"
   grub-install --target=x86_64-efi --bootloader-id=arch --recheck
   ```
   </TabItem>
   <TabItem label="aarch64 (Apple Silicon / ARM)">
   ```bash title="Aarch64 (Apple Silicon / ARM)"
...
3. Generate the GRUB configuration:
   ...
   ```bash title="Grub-mkconfig -o /boot/grub/grub.cfg"
   ...
   ```bash title="UTM aarch64 kernel detection tip"
...
1. Set a hostname:
   ...
   ```bash title="Hostnamectl"
   hostnamectl
   ```

2. Configure `/etc/hosts`:

   ```bash title="Open File for Editing"
   sudo vim /etc/hosts
   ```

   Add these lines:

   ```text title="hosts File Entries"
   127.0.0.1   localhost
...
4. Create a systemd-networkd configuration file:
   ...
   ```ini title="systemd-networkd DHCP Profile"
   [Match]
   Name=enp0s3
...
7. Configure DNS so you can resolve domain names:
   ...
   ```text title="resolv.conf DNS Servers"
   nameserver 1.1.1.1
   nameserver 1.0.0.1
   ```
   ...
   <Aside type="note" title="Optional Production Variant">
      ...
      - If using NetworkManager, let it manage interface addressing and DNS together:
         ...
         ```bash title="Pacman -S Networkmanager"
...
4. From your **host machine** terminal, generate a key pair on your laptop (if you do not already have one):
   ...
   ```bash title="Add a NAT Port-forwarding Rule in Your Hypervisor"
...
5. Copy your host public key into the VM using the forwarded SSH port:
   ...
   ```bash title="Copy SSH Key Over Forwarded Port"
   ...
   ```bash title="Fallback SSH Key Copy"
...
1. Confirm which process is PID 1:
   ...
   ```bash title="Ps -p 1 -o Pid,ppid,comm,args"
...
2. Find PID 2 and identify what it does:
   ...
   ```bash title="Inspect Process IDs"
...
3. Inspect the process tree from the top:
   ...
   ```bash title="Inspect the Process Tree from the Top"
...
2. Confirm your user and group memberships:
   ...
   ```bash title="Check Installed User and Services"
   id
   ```
...
3. Show active services:
   ...
   ```bash title="Show Active Services"
...
4. Confirm network reachability:
   ...
   ```bash title="Ping Arch Linux Mirror"
   ping -c 3 archlinux.org
   ```
