...
<Steps>
1. Start the VM. Select the Arch Linux install medium from the boot menu.

   <Tabs syncKey="arch">
   <TabItem label="x86_64 (Intel/AMD)">
   Select **Arch Linux install medium (x86_64, UEFI)**. After about 30 seconds you will land at a root shell prompt.
   </TabItem>
   <TabItem label="aarch64 (Apple Silicon / ARM)">
   The archboot ISO boots into a setup menu rather than dropping straight to a shell. When you see the menu, press **Ctrl+C** to exit it and reach the root shell prompt.
   </TabItem>
   </Tabs>
