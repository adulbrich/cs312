...
- Network: NAT (default). Keep NAT enabled, then add a host-to-guest port-forwarding rule later so SSH from your laptop is seamless.
...
3. Add a NAT port-forwarding rule in your hypervisor:
   ...
   This maps `127.0.0.1:2222` on your laptop to `22` inside the Arch VM. You keep NAT for outbound internet access, while still being able to SSH in from the host.
...
4. From your **host machine** terminal, generate a key pair on your laptop (if you do not already have one):
   ...
   Accept the default path (`~/.ssh/id_ed25519`) unless you want a course-specific filename.
...
6. Test key authentication from your host:
   ...
   ```bash
   ssh -p 2222 archuser@127.0.0.1 # -i keyfile if you used a non-default filename
   ```

   You should receive a shell prompt without entering the VM password.

7. (Optional) Verify that key login works from inside the VM too:
