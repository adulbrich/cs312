...
3. Generate the GRUB configuration:
   ...
   On some Apple Silicon installs, the kernel file is named `/boot/Image` and GRUB may not detect it automatically. If `grub-mkconfig` does not find Arch, create a compatibility symlink and regenerate the config:
