...
2. In the DNS record form, create an A record:
   ...
   <Aside type="note">
   **Lecture connection:** The lecture's DNS section explains A records: they map a hostname to an IPv4 address. The `.local` suffix is a convention for internal, non-internet-routable domains. In a real company this would be something like `app.company.internal`.
   </Aside>
...
6. Switch to **Simulation mode** and add **DNS** and **HTTP** to the event filter. On the VLAN 20 PC, open **Desktop > Web Browser** and navigate to:
   ...
   <Aside type="note">
   **Lecture connection:** This sequence -- DHCP assigns an address, DNS resolves the name, HTTP fetches the page, all three crossing subnet boundaries through the MLS -- is the complete chain the "Putting It All Together" section of the lecture describes. You are watching it happen one packet at a time.
   </Aside>
...
Here is how each part of this activity maps to the lecture:
...
| Created VLANs and assigned access ports | VLANs and network segmentation |
...
| Deployed a DHCP server and relay agents | DHCP: dynamic addressing |
| Configured internal DNS and created an A record | DNS configuration and troubleshooting; Internal DNS |
| Watched DNS + HTTP packets cross VLAN boundaries in simulation | DNS; Putting it all together |
| Tested DNS failure by pointing clients at a non-existent resolver | DNS troubleshooting |
...
5. **DNS failure modes:** In step 7 of Part 5, browsing to the hostname failed but browsing to the IP address worked. What does this tell you about how DNS fits into the connection chain? When a user says "the website is down," how would this test help you distinguish a DNS failure from an application failure?
