...
5. **Inspect the cluster at a high level:**
   ...
   You should see one node in `Ready` state. The pod list will show around ten pods, but `docker ps` shows only one container on your host: the minikube node itself. Every pod runs inside that single container. Minikube uses the Docker driver to create a node that looks like a real VM to Kubernetes, but from your host it is just a Docker container. The pods inside it are managed by `containerd`, which is the container runtime the kubelet talks to inside the node. Docker on your host is only responsible for the node container, not the workloads inside it.

   Two pods will show `Completed` status: these are Jobs the ingress addon ran once to configure its admission webhook. They finished cleanly, and Kubernetes keeps them so you can read their logs. You will create your own Job later in this activity and see the same lifecycle.
...
2. **Apply the namespace and make it your current default:**
   ...
   The final command should print `cs312-minikube`. From now on, `kubectl` commands in this terminal target that namespace unless you override it.
...
You now have both a stateless and a stateful workload running. This section makes the control loop visible: Pods get replaced, replica counts change, a Horizontal Pod Autoscaler (HPA) object targets the Deployment, and your Service is finally published through an Ingress so the page is reachable from your laptop's browser. An HPA watches a metric (CPU utilization by default) and automatically adjusts a Deployment's replica count to keep that metric within a target range.
...
1. **Temporarily expose the Service as a NodePort:**
   ...
   You should see the HTML page. Notice the port number in the URL: NodePort always allocates from the 30000-32767 range. This is a usable but blunt entry point: every node in the cluster exposes that port whether or not it is running one of the Pods, and clients need to know both the node address and the high-numbered port.
