...
1. **Verify the three tools this activity assumes are already installed:**
   ...
   ```bash title="Verify the Three Tools This Activity Assumes Are Already"
   docker version
   minikube version
...
2. **Create a clean working directory for the manifests you are about to write:**
   ...
   ```bash title="Create a Clean Working Directory for the Manifests You Are"
...
4. **Enable the ingress and metrics-server addons:**
   ...
   ```bash title="Enable Ingress and Metrics Addons"
...
5. **Inspect the cluster at a high level:**
   ...
   ```bash title="Inspect the Cluster at a High Level"
...
6. **Look specifically at the system namespace:**
   ...
   ```bash title="Look Specifically at the System Namespace"
...
7. **Identify which pods are DaemonSet-managed:**
   ...
   ```bash title="Identify Which Pods Are DaemonSet-managed"
...
2. **Apply the namespace and make it your current default:**
   ...
   ```bash title="Apply the Namespace and Make It Your Current Default"
...
4. **Apply the ConfigMap and Secret:**
   ...
   ```bash title="Apply the ConfigMap and Secret"
...
5. **Inspect what was stored:**
   ...
   ```bash title="Inspect What Was Stored"
...
1. **Create `web-deployment.yaml`:**
   ...
   ```yaml title="Rolling Update Strategy"
   apiVersion: apps/v1
   kind: Deployment
   metadata:
     name: orchestra-web
...
2. **Apply the Deployment and wait for it to become ready:**
   ...
   ```bash title="Apply the Deployment and Wait for It to Become Ready"
...
3. **Inspect the workload hierarchy:**
   ...
   ```bash title="Inspect the Workload Hierarchy"
...
4. **Capture one Pod name and confirm its owner:**
   ...
   ```bash title="Capture Web Pod Name"
...
6. **Prove the Service works from inside the cluster by DNS name:**
   ...
   ```bash title="Prove the Service Works from Inside the Cluster by DNS Name"
...
2. **Apply the StatefulSet and wait for it to be ready:**
   ...
   ```bash title="Apply the StatefulSet and Wait for It to Be Ready"
...
3. **Inspect the objects Kubernetes created for you:**
   ...
   ```bash title="Inspect the Objects Kubernetes Created for You"
...
4. **Write your own content into the mounted volume:**
   ...
   ```bash title="Write Your Own Content into the Mounted Volume"
...
5. **Read that same file back through the Pod's stable DNS name:**
   ...
   ```bash title="Read That Same File Back Through the Pod's Stable DNS Name"
...
6. **Delete the Pod and watch the StatefulSet replace it:**
   ...
   ```bash title="Delete the Pod and Watch the StatefulSet Replace It"
...
7. **Confirm that the file survived the Pod replacement:**
   ...
   ```bash title="Confirm That the File Survived the Pod Replacement"
...
2. **Apply the Job and watch the Pod move through its lifecycle:**
   ...
   ```bash title="Apply the Job and Watch the Pod Move Through Its Lifecycle"
...
3. **Read the output and inspect the Job's final state:**
   ...
   ```bash title="Inspect Job Logs and Status"
...
1. **Delete one web Pod and watch the Deployment heal the gap:**
   ...
   ```bash title="Delete One Web Pod and Watch the Deployment Heal the Gap"
...
2. **Scale the Deployment manually to three replicas:**
   ...
   ```bash title="Scale the Deployment Manually to Three Replicas"
...
3. **Check that metrics-server is feeding live resource data:**
   ...
   ```bash title="Check That Metrics-server Is Feeding Live Resource Data"
   kubectl top pods
   ```
...
5. **Apply the HPA and Ingress, then inspect the autoscaler object:**
   ...
   ```bash title="Apply HPA and Ingress"
...
7. **Roll the Deployment so new Pods pick up the new desired content:**
   ...
   ```bash title="Restart Web Deployment"
...
8. **Now make one Pod-template change that the Deployment can actually roll back:**
   ...
   ```bash title="web-deployment.yaml"
   kubectl apply -f web-deployment.yaml
   kubectl rollout status deployment/orchestra-web
   ```
...
9. **Roll back to the previous revision:**
   ...
   ```bash title="Roll Back to the Previous Revision"
...
10. **Confirm the image rolled back and the page still shows the updated message:**
    ...
    ```bash title="Verify Rollback Result"
...
1. **Temporarily expose the Service as a NodePort:**
   ...
   ```bash title="Temporarily Expose the Service as a NodePort"
...
2. **Switch back to ClusterIP and use Ingress instead:**
   ...
   ```bash title="Switch Back to ClusterIP and Use Ingress Instead"
...
3. **Start the minikube tunnel in a second terminal and leave it running:**
   ...
   ```bash title="Start Minikube Tunnel"
   minikube tunnel
   ```
...
2. **Delete the activity namespace and everything in it:**
   ...
   ```bash title="Delete the Activity Namespace and Everything in It"
...
3. **Reset your current `kubectl` namespace to `default`:**
   ...
   ```bash title="Reset Your Current Kubectl Namespace to Default"
