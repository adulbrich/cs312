...
1. **Temporarily expose the Service as a NodePort:**
   ...
   minikube service orchestra-web -n orchestration-lab --url
   ```
   ...
   You should see the HTML page. Pass `-n orchestration-lab` explicitly here rather than assuming minikube will reuse your current `kubectl` namespace. Notice the port number in the URL: it is in the 30000-32767 range, which is what the lecture describes as the default NodePort range. This is a usable but blunt entry point: every node exposes the same port, regardless of whether it is running one of the Pods.
...
2. **Delete the activity namespace and everything in it:**
   ...
   ```bash
   kubectl delete namespace orchestration-lab --wait=true
   ```
