...
11. Install the AWS CLI v2 and a TCP probe tool, then confirm the CLI uses the role without any configuration:
    ...
    Ubuntu 24.04 does not ship `awscli` in its apt repositories, so you install it from the official AWS bundle instead. The response shows your account ID and the role ARN. The CLI found and used the credentials from the metadata service without a single `aws configure` prompt.
