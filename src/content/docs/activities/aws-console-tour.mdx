...
{/* ai-summary
type: activity
slug: aws-console-tour
order: 6
paired_lecture: cloud-computing
practices: VPC and subnet CIDR inspection; public vs private subnet routing (IGW vs local); security group inbound and outbound rules; IAM role creation and EC2 instance attachment; instance metadata service for credential retrieval; S3 bucket creation and object upload using role credentials; ECR repository creation and docker push with no credentials on disk
prereq_activities:
output: a two-tier VPC with a public EC2 probe host and a private EC2 instance, an S3 object uploaded using role credentials, and a container image pushed to ECR with no credentials stored on any instance
*/}
...
<Aside type="note" title="How this topology differs from the lecture">
The lecture describes a three-tier production pattern: a public load balancer tier, a private application tier, and a private data tier, with outbound-only internet access through a NAT gateway and no direct SSH into any instance. This activity simplifies that into two subnets and uses a direct SSH probe host in the public subnet. The route table and security group mechanics are identical to production; the difference is operational convenience for class, not a different architecture.
</Aside>
...
<Aside type="note" title="Network ACLs: a second, stateless layer">
Every subnet in a VPC is automatically associated with a Network ACL (NACL). Navigate to **VPC > Network ACLs** and find the default NACL covering your new subnets. Unlike security groups, NACLs are stateless: an inbound allow rule does not automatically permit the return traffic. You must explicitly allow the ephemeral port range (1024-65535) outbound for reply packets to reach the client. Rules are evaluated in number order and the first match wins; the default NACL ends with a catch-all allow rule on both directions, which is why most deployments leave it untouched and do the real filtering at the security group layer.
</Aside>
...
3. Launch a second instance that plays the private-tier role.
   ...
   ```bash title="User Data TCP Listener"
   #!/bin/bash
...
6. On your local machine, load your key into the SSH agent so agent forwarding works later, then connect to `cs312-web`:
   ...
   ```bash title="Ssh-add /path/to/your-key.pem"
...
9. Query the IAM path to find the name of the attached role:
   ...
   ```bash title="Read Instance Role Name"
...
12. Validate the security-group relationship you built earlier by probing the private instance on port 5432:
    ...
    ```bash title="Probe Private Database Port"
...
14. From inside `cs312-web`, SSH to the private instance using only its private IP:
    ...
    ```bash title="SSH to Private Instance"
...
<ActivityQuestion>
The `aws s3 cp` commands in steps 2 and 4 succeeded with no credential configuration on this instance. Trace the path the credentials took: starting from the IAM role, through the metadata service, to the AWS CLI, to the S3 API request. Name each step. Then explain why this path means you can replace this instance with a new one and the new instance will have working credentials from the moment it starts. Finally, in step 8 you saw the role's allow overridden by the bucket policy's deny. What does this tell you about which layer, identity-based or resource-based, has the final word when both apply to the same request?
</ActivityQuestion>
...
7. Tag the image with both a version tag and `latest`, then push both:
   ...
   ```bash title="Docker Tag Cs312-youronid:v1"
...
8. In the AWS Console, refresh your ECR repository page. You should see both `v1` and `latest` listed, sharing the same digest. Click either tag and open the **Scan results** tab.
   ...
   ```bash title="Inspect ECR Repository"
...
If the scan results showed a CRITICAL severity finding in the `nginx:alpine` base image, what would your next steps be? Name at least two concrete actions: one for the image already in the registry and one for preventing the same finding from reaching production in the future.
</ActivityQuestion>
...
- Add a NAT gateway to `cs312-vpc` so instances in `private-subnet` can make outbound requests (software updates, package downloads) without being directly reachable from the internet. Follow the [NAT gateway documentation](https://docs.aws.amazon.com/vpc/latest/userguide/vpc-nat-gateway.html) for the exact steps. Notice the tradeoff from the lecture: NAT gateways are the standard managed path, but they also incur hourly and data-processing charges.
