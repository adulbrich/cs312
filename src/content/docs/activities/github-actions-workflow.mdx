...
7. **Commit and push everything:**
   ...
   ```bash title="Commit and Push Everything"
...
5. **Expand the Lint and Test steps.** The Lint step should complete silently: no output means `flake8` found no issues. The Test step should print something like:
   ...
   ```text title="Expand the Lint and Test Steps Output"
...
1. **Add the `build` job** to `.github/workflows/ci.yml`. Append these lines after the last line of the `quality` job block, at the same indentation level as `quality:` under `jobs:`:
   ...
   ```yaml title="Set Up Docker Buildx Step"
     build:
       needs: quality
...
4. **Expand the Smoke-test step** inside the `build` job. You will see `docker run` start the container, followed by `curl` output showing the JSON response from your application:
   ...
   ```text title="Expand the Smoke-test Step Output"
...
6. **Push a small change** and watch the layer cache in action:
   ...
   ```bash title="Push a Small Change"
...
2. **Add an upload step** at the end of the `quality` job's `steps:` list, after the Test step:
   ...
   ```yaml title="Upload Test Report Step"
...
3. **Open the Actions tab.** Both `quality (3.11)` and `quality (3.12)` jobs will fail. Click into either run and expand the **Lint** step. You will see output like:
   ...
   ```text title="Open the Actions Tab Output"
...
4. **Fix the file.** Remove `import os`, leaving only the original two lines:
   ...
   ```python title="Fix the File"
   from flask import Flask, jsonify
   ```

5. **Commit and push the fix:**

   ```bash title="Commit and Push the Fix"
...
```yaml title="Add Concurrency Control to the Build Job"
  build:
    needs: quality
    if: github.event_name == 'push' && github.ref == 'refs/heads/main'
    concurrency:
...
```yaml title="Push to Amazon ECR Using OIDC Instead of GHCR"
    permissions:
      contents: read
      id-token: write
```
...
```yaml title="Configure AWS Credentials Step"
