...
<Aside type="caution" title="Lab 7 Data May Be Gone">
...
1. **Write the PVC manifest**
   ...
   ```bash title="Write the PVC Manifest"
   cd ~/k8s-lab
...
4. **Apply the updated Deployment**
   ...
   <Aside type="note" title="PVC Starts Empty">
...
5. **Verify the PVC is bound**
   ...
   ```bash title="Verify the PVC Is Bound"
   kubectl get pvc
   ```
...
6. **Verify data persists across pod restarts**
   ...
   ```bash title="Verify Data Persists Across Pod Restarts"
...
4. **Verify Pods are Running and Ready**
   ...
   ```bash title="Verify Pods Are Running and Ready"
   kubectl get pods
   ```
...
1. **Record the current revision**
   ...
   ```bash title="Record the Current Revision"
...
2. **Pause the Deployment before making changes**
   ...
   ```bash title="Pause the Deployment Before Making Changes"
...
3. **Update the image while paused**
   ...
   ```bash title="Update the Image While Paused"
...
4. **Add a change-cause annotation (common operator practice)**
   ...
   ```bash title="Add a Change-cause Annotation (Common Operator Practice)"
...
5. **Verify the Deployment is paused**
   ...
   ```bash title="Verify the Deployment Is Paused"
...
6. **Resume and watch the rollout begin**
   ...
   ```bash title="Resume and Watch the Rollout Begin"
...
7. **Wait for rollout completion**
   ...
   ```bash title="Wait for Rollout Completion"
...
8. **Verify the new image**
   ...
   ```bash title="Verify the New Image"
...
9. **Check the revision history**
   ...
   ```bash title="Check the Revision History"
   kubectl rollout history deployment/nginx
   ```
...
2. **Verify the rollback**
   ...
   ```bash title="Verify the Rollback"
   kubectl describe deployment nginx | grep Image
   ```
...
1. **Deploy a nonexistent image**
   ...
   ```bash title="Deploy a Nonexistent Image"
   kubectl set image deployment/nginx nginx=nginx:doesnotexist
   ```

2. **Observe the failure**

   ```bash title="Observe the Failure"
   kubectl get pods
   ```
   ...
   <Aside type="note" title="ImagePullBackOff Retries">
...
3. **Diagnose the failure**
   ...
   ```bash title="Diagnose the Failure"
...
4. **Recover via rollback**
   ...
   ```bash title="Recover Via Rollback"
   kubectl rollout undo deployment/nginx
   ```
...
2. **Apply the change**
   ...
   ```bash title="Apply the Change"
   kubectl apply -f nginx-deployment.yaml
   ```

3. **Observe the failure pattern**

   ```bash title="Observe the Failure Pattern"
   kubectl get pods -w
   ```
...
4. **Diagnose the failure (authoritative step)**
   ...
   ```bash title="Diagnose the Failure (Authoritative Step)"
...
5. **Check logs when available**
   ...
   ```bash title="Check Logs When Available"
