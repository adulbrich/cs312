...
2. How many revisions does `kubectl rollout history deployment/nginx` show after the rolling update? What image tag was used in the most recent revision? *(3 points)*
3. During the rolling update, did you observe both old and new Pods running at the same time? How does Kubernetes ensure zero downtime during a rollout? *(5 points)*
...
6. After your final rollback, what image tag are the healthy Pods running? How many Pods show `1/1 Ready`? *(2 points)*
...
2. **Update the nginx Deployment to add probes and resource constraints**
   ...
   - **`replicas: 2`**: We intentionally run two nginx Pods in this lab so you can observe high-availability behavior during updates and failures. With one replica, a failed readiness check or restart can briefly remove all proxy capacity. With two replicas, one Pod can be removed from traffic while the other keeps serving requests, making rolling updates and probe behavior easier to see.
