...
{/* TODO: convert this to a mermaid diagram */}
```text title="Control Node to Managed Node Topology"
┌──────────────┐         SSH          ┌──────────────┐
│              │ ──────────────────▶  │              │
...
<Aside type="note" title="Alternative: Run Ansible from Your Laptop">
If you prefer to run Ansible directly from your laptop instead of SSH-ing into the control node, you can (but support varies by OS):

- **macOS**: `brew install ansible`. Use your laptop as the control node and only the managed node EC2 instance.
- **Linux**: `sudo apt update && sudo apt install -y ansible`. Same as macOS.
- **WSL2 on Windows**: Install Ansible inside WSL2 the same way as Linux. Your `.pem` key is likely in the Windows filesystem; reference it with a WSL2 path like `/mnt/c/Users/<you>/Downloads/cs312-key.pem`.
- **Windows (native)**: Ansible does not run natively on Windows. Use WSL2, or use the EC2 control node approach described in this lab.

If using your laptop, skip the "Setting Up the Control Node" section and use the managed node's **public IP** (instead of private IP) in your inventory, since you will not be on the same VPC. All other steps are identical.
</Aside>

### Using Your Terraform Infrastructure

Your two EC2 instances were provisioned by the Terraform configuration in the previous lab. From your laptop, re-apply if the instances are not already running:

```bash title="Windows (Native)"
cd ~/terraform-lab
terraform apply -var="key_name=cs312-key"
```

The outputs will show four values you will need throughout this lab:

```text title="Using Your Terraform Infrastructure Output"
control_node_public_ip  = "54.x.x.x"
managed_node_public_ip  = "54.x.x.x"
managed_node_private_ip = "172.x.x.x"
ecr_repository_url      = "471112664344.dkr.ecr.us-east-1.amazonaws.com/cs312-wordpress-lab"
```

The **control node** is where you will install Ansible and run playbooks. The **managed node** is what Ansible will configure; WordPress will run there. Both instances use your `cs312-key.pem` key pair. The **ECR repository URL** is needed in Part B when configuring the GitHub Actions pipeline.
...
1. **SSH into your control node**
   ...
   ```bash title="SSH into Your Control Node"
   ssh -i ~/Downloads/cs312-key.pem ubuntu@<control-node-public-ip>
   ```

2. **Install Ansible**

   ```bash title="Install Ansible"
...
3. **Copy your SSH key to the control node**
   ...
   ```bash title="Copy Your SSH Key to the Control Node"
   scp -i ~/Downloads/cs312-key.pem ~/Downloads/cs312-key.pem ubuntu@<control-node-public-ip>:~/cs312-key.pem
   ```
...
4. **Verify SSH connectivity to the managed node**
   ...
   ```bash title="Verify SSH Connectivity to the Managed Node"
   ssh -i ~/cs312-key.pem ubuntu@<managed-node-private-ip>
   ```

   <Aside type="tip" title="Use the Managed Node Private IP">
...
2. **Write an Ansible configuration file**
   ...
   <Aside type="note" title="Host Key Checking Is Lab-Only">
...
3. **Write the inventory file**
   ...
   ```bash title="Edit inventory"
   vim inventory
   ```

   ```ini title="Ansible Inventory"
   [webservers]
   wordpress ansible_host=<managed-node-private-ip> ansible_user=ubuntu ansible_ssh_private_key_file=~/cs312-key.pem
   ```
...
4. **Test connectivity with Ansible**
   ...
   ```bash title="Test Connectivity with Ansible"
   ansible webservers -m ping
   ```

   You should see output like:

   ```text title="Writing the Inventory and Playbook Output"
   wordpress | SUCCESS => {
       "changed": false,
       "ping": "pong"
   }
   ...
   <Aside type="caution" title="Troubleshooting UNREACHABLE">
   If you see "UNREACHABLE," double-check: (1) the private IP in your inventory, (2) the key file path, (3) that the managed node's security group allows SSH from the control node's security group.
   </Aside>
...
5. **Write the playbook**
   ...
   <Aside type="caution" title="Plaintext Playbook Secrets Are Lab-Only">
   Hardcoding passwords directly in a playbook is acceptable for a lab but is a serious security risk in production. Anyone with read access to the file can see the credentials, and they end up in Git history if the file is committed. The proper solution is **Ansible Vault**, which encrypts sensitive values so you can safely commit them. A simpler alternative is to pass variables at runtime with `-e "mysql_root_password=..."` so they never touch disk. For this lab, plaintext vars are fine.
   </Aside>
...
6. **Install the Docker collection**
   ...
   ```bash title="Install the Docker Collection"
...
9. **Run the playbook again to prove idempotency**
   ...
   ```bash title="Run the Playbook Again to Prove Idempotency"
   ansible-playbook configure.yml
   ```
...
1. **Create a new GitHub repository**
   ...
   ```bash title="Create a New GitHub Repository"
...
3. **Write the GitHub Actions workflow**
   ...
   ```bash title="Write the GitHub Actions Workflow"
   mkdir -p .github/workflows
   vim .github/workflows/build-push.yml
   ```

   ```yaml title="Checkout Code Step"
   ...
   - **Steps**:
    ...
    <Aside type="note" title="Why This Lab Uses AWS Secrets Instead of OIDC">
    In a production AWS environment, GitHub Actions would usually authenticate with AWS by using **OpenID Connect (OIDC)** and assuming an IAM role. That avoids storing cloud credentials in GitHub.

    AWS Academy Learner Lab does not permit students to create the IAM OIDC provider required for that setup. In testing, attempts to create `token.actions.githubusercontent.com` were denied with `iam:CreateOpenIDConnectProvider`. Because of that limitation, this lab uses the temporary AWS Academy credentials from the **AWS Details** page and stores them as GitHub Actions secrets instead.

    If you later work in a full AWS account, replace this secrets-based step with an OIDC-based role assumption flow.
    </Aside>
...
4. **Configure repository secrets**
   ...
   <Aside type="caution" title="Refresh GitHub AWS Secrets">
...
6. **Create and push a tag**
   ...
   ```bash title="Create and Push a Tag"
   git tag v1.0.0
   git push --tags
   ```
...
7. **Verify the image in ECR**
   ...
   ```bash title="Verify the Image in ECR"
...
1. **Destroy the AWS infrastructure**
   ...
   From your laptop, in the Terraform project directory:

   ```bash title="Destroy the AWS Infrastructure"
   cd ~/terraform-lab
   terraform destroy -var="key_name=cs312-key"
   ```

   Type `yes` to confirm. This removes the two EC2 instances, both security groups, and the ECR repository. The ECR destroy will fail if the repository still contains images; delete them first via the AWS Console (ECR > Repositories > `cs312-wordpress-lab` > select all images > Delete) or with the CLI:

   ```bash title="AWS ECR Batch-delete-image"
   aws ecr batch-delete-image \
     --repository-name cs312-wordpress-lab \
...
2. **Delete the GitHub repository**
   ...
   Delete the pipeline repository from GitHub: go to the repository, then **Settings > Danger Zone > Delete this repository**.
