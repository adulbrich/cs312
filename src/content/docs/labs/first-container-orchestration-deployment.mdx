...
4. After configuring nginx as a reverse proxy and the Ingress, what does `curl -L http://localhost` return? Describe the redirect chain: what responds first, what does it redirect to, and what serves the final response? *(4 points)*
...
7. When you first encountered the `?reauth=1` redirect loop, what command showed that no `WORDPRESS_AUTH_KEY` environment variables were set in the Pod? What is the root cause of the loop with two replicas? *(4 points)*
8. After applying the updated Secret and Deployment, what did `kubectl rollout status deployment/wordpress` report? How does a rolling update keep the site available during the update? *(4 points)*
...
nginx will be the only publicly-accessible entry point. It forwards requests from Traefik (k3s's built-in ingress controller, running on port 80) to `wordpress-service`. The proxy configuration lives in a ConfigMap, so you can update routing rules without rebuilding the nginx image.
...
```
Browser → port 80 → Traefik (Ingress) → nginx-service → nginx Pod → wordpress-service → WordPress Pod (Apache + PHP) → mariadb-service → MariaDB Pod
```

This separation is common in production: nginx handles load balancing across multiple upstream servers and connection management, while the application server focuses on running application code. Traefik handles the public port 80 entry point via an Ingress resource.
...
4. **Apply the changes**
   ...
   Kubernetes performs a **rolling update**: it starts a new Pod with the updated environment variables, waits for it to become ready, then terminates an old Pod, and repeats. At no point are all running Pods shut down simultaneously, so the site stays available throughout. Record what `kubectl rollout status` reports for your lab questions.
