...
1. **Launch an EC2 instance**
   ...
   ```bash title="Launch an EC2 Instance"
   ssh -i ~/Downloads/cs312-key.pem ec2-user@<your-public-ip>
   ```

   <Aside type="caution" title="Use a Plain SSH Session">
...
3. **Configure kubectl access**
   ...
   ```bash title="/etc/rancher/k3s/k3s.yaml"
   mkdir -p ~/.kube
   ...
   <Aside type="caution" title="Protect Cluster-Admin Kubeconfig">
...
1. **Create a project directory and write the Secret**
   ...
   ```bash title="Create a Project Directory and Write the Secret"
...
2. **Apply the Secret**
   ...
   ```bash title="Apply the Secret"
   kubectl apply -f db-secret.yaml
   ```

3. **Inspect the stored value**

   ```bash title="Inspect the Stored Value"
...
<Aside type="caution" title="Ephemeral Database Storage">
...
1. **Write the MariaDB Deployment**
   ...
   ```bash title="Edit mariadb-deployment.yaml"
...
2. **Write the MariaDB Service**
   ...
   ```bash title="Edit mariadb-service.yaml"
...
1. **Write the WordPress Deployment**
   ...
   ```bash title="Edit wordpress-deployment.yaml"
...
2. **Write the WordPress Service**
   ...
   ```bash title="Edit wordpress-service.yaml"
...
4. **Verify the WordPress Pods**
   ...
   ```bash title="Verify the WordPress Pods"
...
```mermaid title="Deploying Nginx as a Reverse Proxy Diagram"
flowchart TB
  browser[Browser]
...
1. **Create the nginx ConfigMap**
   ...
   ```bash title="Edit nginx-configmap.yaml"
...
2. **Write the nginx Deployment**
   ...
   ```bash title="Edit nginx-deployment.yaml"
...
3. **Write the nginx Service**
   ...
   ```bash title="Edit /etc/nginx/conf.d/default.conf"
   ...
   <Aside type="note" title="Prefer Ingress Over NodePort">
...
4. **Write the Ingress**
   ...
   ```bash title="Edit wordpress-ingress.yaml"
...
1. **Confirm both Pods are running**
   ...
   ```bash title="Confirm Both Pods Are Running"
...
2. **Check logs from both Pods simultaneously**
   ...
   ```bash title="Check Logs from Both Pods Simultaneously"
...
3. **Confirm no auth keys are set**
   ...
   ```bash title="Confirm No Auth Keys Are Set"
...
1. **Generate the eight auth keys**
   ...
   ```bash title="Generate the Eight Auth Keys"
...
3. **Update the WordPress Deployment**
   ...
   ```yaml title="wordpress-deployment.yaml"
...
1. **List the running WordPress Pods**
   ...
   ```bash title="List the Running WordPress Pods"
   kubectl get pods -l app=wordpress
   ```
...
2. **Delete one WordPress Pod**
   ...
   ```bash title="Delete One WordPress Pod"
...
3. **Watch the replacement**
   ...
   ```bash title="Watch the Replacement"
   kubectl get pods -l app=wordpress
   ```
...
4. **Describe the WordPress Service endpoints**
   ...
   ```bash title="Describe the WordPress Service Endpoints"
