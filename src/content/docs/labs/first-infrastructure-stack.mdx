...
{/* TODO: this should go more in-depth than the activity, with "production-grade" best practices, put the code in a git repository, use S3 for the state, etc. */}
{/* TODO: refer to the terraform docs for installation instead of writing our own, consider mentioning the AWS CLI as well, make this standalone to the activity */}
...
1. **Install Terraform on your laptop**
   ...
   **Windows (with Chocolatey):**
   ```bash title="Windows (with Chocolatey)"
...
3. **Configure AWS credentials**
   ...
   ```ini title="Terraform HashiCorp Repository"
   [default]
   ...
   ```ini title="Install Terraform Package"
   [cs312]
   aws_access_key_id=ASIA...
   aws_secret_access_key=...
   aws_session_token=...
...
2. **Write `main.tf`**
   ...
   ```bash title="Edit main.tf"
   vim main.tf
   ```

   Add the following:

   ```hcl title="Terraform AWS Provider"
   terraform {
     required_providers {
       aws = {
...
3. **Write `variables.tf`**
   ...
   <Aside type="tip" title="AMI IDs Change">
   The `ami_id` default may need updating; AMI IDs change over time and vary by region. To find the current Ubuntu 26.04 LTS AMI in us-east-1, open the EC2 Console, click **Launch instance**, search for "Ubuntu 26.04", and copy the AMI ID from the results without actually launching anything. It will look like `ami-0d13e2317a7e75c95`. Paste it as the default in `variables.tf`.

   If you have the AWS CLI configured, you can also retrieve it programmatically (add `--profile profile-name` if you are using a named profile):
...
1. **Initialize the project**
   ...
   ```bash title="Initialize the Project"
   terraform init
   ```
...
2. **Preview the changes**
   ...
   ```bash title="Preview the Changes"
...
4. **Verify by SSHing in**
   ...
   ```bash title="Verify by Sshing in"
...
2. **Preview the change**
   ...
   <Aside type="note" title="Plan Is Enough Here">
...
3. **Provision a dedicated VPC and network stack**
   ...
   ```hcl title="Next, Update the Vpcid in Both Security Groups to Reference"
   ...
   ```hcl title="Dedicated VPC Resources"
   ...
   ```bash title="Preview Terraform Changes"
   terraform plan -var="key_name=cs312-key"
   ```
   ...
   <Aside type="note" title="VPC Changes Cascade">
...
1. **Inspect the state file**
   ...
   ```bash title="Inspect the State File"
   ...
   <Aside type="danger" title="State Files Are Sensitive">
...
1. **Tear everything down**
   ...
   ```bash title="Tear Everything Down"
