---
title: 'Image Registry, Backups, and Version Switching (ECR + S3)'
...
<Aside type="note" title="Free Disk Space Before Starting">
If you skipped the Lab 3 cleanup step, run `docker system prune -a` before continuing to ensure you have enough disk space. Named volumes are unaffected.
</Aside>
...
2. **Configure AWS credentials**
   ...
   1. In the Learner Lab portal, click **AWS Details**
   2. Click **Show** next to **AWS CLI**
   3. Copy the credentials block (it contains `aws_access_key_id`, `aws_secret_access_key`, and `aws_session_token`)
   4. On your EC2 instance, paste them into the credentials file:
   ...
   The file should look like this (with your actual values from the portal):

   ```ini title="The File Should Look Like This (with Your Actual Values from"
   [default]
   ...
   Also set the default region:

   ```bash title="AWS Configure Set Region Us-east-1"
   aws configure set region us-east-1
   ```

   <Aside type="caution" title="Refresh AWS Academy Credentials">
   AWS Academy credentials expire when your Learner Lab session ends. Each time you start a new lab session, you must repeat this step with the fresh credentials shown in the portal.
   </Aside>

3. **Create a repository via the CLI**

   ```bash title="Create a Repository Via the CLI"
...
2. **Authenticate Docker to ECR**
   ...
   ```bash title="Authenticate Docker to ECR"
   ...
   <Aside type="tip" title="ECR Login Tokens Expire">
...
1. **Pull and tag version 1**
   ...
   ```bash title="Pull and Tag Version 1"
   docker pull wordpress:6.4.3
   docker run --rm wordpress:6.4.3 grep wp_version /usr/src/wordpress/wp-includes/version.php
   docker tag wordpress:6.4.3 <your-repo-uri>:wp-6.4-v1
   docker push <your-repo-uri>:wp-6.4-v1
   ```

   Replace `<your-repo-uri>` with your full ECR repository URI. The `grep` line confirms the image actually contains the version you expect before pushing; minor version tags like `6.4` are floating and can drift to newer patches over time. The WordPress entrypoint only copies files to `/var/www/html` when starting Apache, so the pre-push check reads directly from `/usr/src/wordpress`, where the image stores its source files.

2. **Pull and tag version 2**

   Repeat with a different WordPress version:

   ```bash title="Pull and Tag Version 2"
   docker pull wordpress:6.5.5
   docker run --rm wordpress:6.5.5 grep wp_version /usr/src/wordpress/wp-includes/version.php
   docker tag wordpress:6.5.5 <your-repo-uri>:wp-6.5-v2
   docker push <your-repo-uri>:wp-6.5-v2
   ```

3. **Verify both images are in ECR**

   ```bash title="Verify Both Images Are in ECR"
...
1. **Create a Compose file using ECR images**
   ...
   ```bash title="Create a Compose File Using ECR Images"
...
2. **Start the stack and create content**
   ...
   Then switch to a theme that works across both WordPress versions. In the WordPress admin (**Appearance → Themes**), activate **Twenty Twenty-Four**. This avoids a crash when rolling back to 6.4, because the Twenty Twenty-Five theme (bundled with 6.5) uses an API that does not exist in 6.4.

3. **Switch to version 2**

   Edit `docker-compose.yml` and change the WordPress image tag from `wp-6.4-v1` to `wp-6.5-v2`:

   ```bash title="Edit docker-compose.yml"
   vim docker-compose.yml
   ```

   Then pull the new image from ECR and apply the change:

   ```bash title="Docker Compose Pull"
   docker compose pull
   docker compose up -d -V
   ```

   Two flags matter here. `docker compose pull` fetches the image from ECR rather than using whatever is cached locally. The `-V` (`--renew-anon-volumes`) flag tells Compose to discard the anonymous volume at `/var/www/html` that the WordPress image creates behind the scenes. Without `-V`, Docker copies the old container's anonymous volume data into the new container, and the WordPress entrypoint sees the old version's files already in place and skips the update. Your named volumes (`db_data` and `wp_content`) are unaffected by `-V`, so your blog post and database survive the switch.

   Verify the WordPress version inside the container:

   ```bash title="Verify the WordPress Version Inside the Container"
...
4. **Roll back to version 1**
   ...
   ```bash title="Roll Back to Version 1"
   docker compose pull
   docker compose up -d -V
   ```
...
2. **Dump the database**
   ...
   ```bash title="Dump the Database"
   docker exec $(docker compose ps -q db) \
     mariadb-dump -u root -p"$(grep MYSQL_ROOT_PASSWORD .env | cut -d= -f2)" wordpress > backup.sql
   ```

   This creates a SQL file on your EC2 instance containing every table, row, and setting in the WordPress database.

   <Aside type="tip" title="Verify the Database Backup">
   Check the backup is not empty: `wc -l backup.sql` should show hundreds or thousands of lines. If it shows fewer than 10, your dump command had an error (check the password) or WordPress has not been set up yet (complete the installation wizard first).
   </Aside>
3. **Upload to S3**
...
4. **Configure a lifecycle rule**
   ...
   ```bash title="Configure a Lifecycle Rule"
...
1. **Destroy the database volume**
   ...
   ```bash title="Destroy the Database Volume"
...
2. **Start fresh containers**
   ...
   ```bash title="Start Fresh Containers"
   docker compose up -d
   ```
...
3. **Download the backup from S3**
   ...
   ```bash title="Download the Backup from S3"
   aws s3 cp s3://cs312-<your-username>-backups/backups/backup.sql ./restore.sql
   ```

4. **Import the backup**

   ```bash title="Import the Backup"
   docker exec -i $(docker compose ps -q db) \
     mariadb -u root -p"$(grep MYSQL_ROOT_PASSWORD .env | cut -d= -f2)" wordpress < restore.sql
   ```
...
```bash title="Verify the Restore"
docker compose down
```

Delete the ECR repository and S3 bucket as well to avoid charges:

```bash title="Delete Registry and Backup Resources"
