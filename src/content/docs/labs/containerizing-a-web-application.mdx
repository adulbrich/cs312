...
1. **Launch a fresh EC2 instance**
   ...
   In the AWS Console, launch an Ubuntu Server 24.04 LTS instance (t3.micro). Reuse your existing key pair (`cs312-key`) and Security Group (`cs312-wordpress-sg`, which already allows SSH on port 22 and HTTP on port 80). Connect via SSH:
...
If you want to preserve the data volumes for a future session, do **not** add `-v`. The next lab will build on this one, so you can keep instance around (**stop** instance) and reuse it. If you want to start fresh next time, you can terminate the EC2 instance. You will have to launch a new one and repeat the Docker installation step, but you can reuse your existing key pair and security group.
