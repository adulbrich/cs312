...
{/* Validated Time-to-Complete: ~1:00 */}
{/* TODO: this lab is too short and needs to be revised to go more in-depth compared to the activity but still stay aligned with the premise */}
...
1. **Launch a fresh EC2 instance**
   ...
   In the AWS Console, launch an Ubuntu Server 26.04 LTS instance (t3.micro). Reuse your existing key pair (`cs312-key`) and Security Group (`cs312-wordpress-sg`, which already allows SSH on port 22 and HTTP on port 80). Under **Configure storage**, increase the root volume to **20 GiB** (the default 8 GiB fills up quickly when pulling multiple Docker images). Connect via SSH:

   ```bash title="Launch a Fresh EC2 Instance"
...
3. **Add your user to the docker group**
   ...
   ```bash title="Add Your User to the Docker Group"
   ...
   <Aside type="caution" title="Docker Group Grants Root">
...
4. **Verify Docker is working**
   ...
   ```bash title="Verify Docker Is Working"
   docker --version
...
2. **Create an environment file for credentials**
   ...
   ```bash title="Create an Environment File for Credentials"
   vim .env
   ```

   Add the following (choose your own passwords):

   ```text title="WordPress .env Values"
   ...
   <Aside type="caution" title=".env Files Are Secrets">
...
2. **Check the containers are running**
   ...
   ```bash title="Check the Containers Are Running"
   docker compose ps
   ```
...
1. **Stop and restart without removing volumes**
   ...
   ```bash title="Stop and Restart without Removing Volumes"
   docker compose down
   docker compose up -d
   ```
...
2. **Stop and restart WITH volume removal**
   ...
   ```bash title="Stop and Restart WITH Volume Removal"
   ...
   <Aside type="danger" title="down -v Deletes Volumes">
   ...
   ```bash title="External volumes (optional)"
   docker volume create wordpress_db_data
   ```

   ```yaml title="External Compose Volume"
   volumes:
     db_data:
       external: true
...
3. **Check that containers restarted**
   ...
   ```bash title="Check That Containers Restarted"
...
1. **Open a shell inside the WordPress container**
   ...
   ```bash title="Open a Shell Inside the WordPress Container"
   docker exec -it $(docker compose ps -q wordpress) bash
   ```

   <Aside type="note" title="Resolving the WordPress Container">
   ...
   ```bash title="Exec into Container by Name"
...
2. **Find the WordPress version**
   ...
   ```bash title="Find the WordPress Version"
