...
1. **Update the package index**
   ...
   ```bash title="Update the Package Index"
...
<Aside type="tip" title="Choose RDS for This Lab">
...
3. **Note the RDS endpoint**
   ...
   ```text title="Note the RDS Endpoint Output"
...
4. **Create the WordPress database**
   ...
   ```bash title="Create the WordPress Database"
   ...
   ```sql title="Option 1: Setting Up an RDS MariaDB Instance SQL"
...
<Aside type="caution" title="Delete RDS After the Lab">
...
```text title="RDS Endpoint Format"
...
1. **Create a database security group**
   ...
   <Aside type="note" title="Keep the Database Private">
...
4. **Connect via SSH and install MariaDB**
   ...
   ```bash title="Connect Via SSH and Install MariaDB"
...
6. **Configure MariaDB to accept remote connections**
   ...
   ```bash title="Configure MariaDB to Accept Remote Connections"
   sudo vim /etc/mysql/mariadb.conf.d/50-server.cnf
   ```

   Find the line:

   ```ini title="Find the Line"
   bind-address            = 127.0.0.1
   ```

   Change it to:

   ```ini title="Change It to"
...
7. **Create the WordPress database and user**
   ...
   ```bash title="Create the WordPress Database and User"
   sudo mysql -u root -p
   ```
   ...
   ```sql title="Create WordPress Database and User"
   CREATE DATABASE wordpress;
   CREATE USER 'wp_user'@'%' IDENTIFIED BY 'your-wp-password-here';
   GRANT ALL PRIVILEGES ON wordpress.* TO 'wp_user'@'%';
   FLUSH PRIVILEGES;
...
8. **Test the connection from the WordPress instance**
   ...
   ```bash title="Test the Connection from the WordPress Instance"
   sudo apt install -y mariadb-client
...
```text title="Database Host Format"
...
1. **Create a dedicated OS user**
   ...
   ```bash title="Create a Dedicated OS User"
...
4. **Configure WordPress**
   ...
   ```bash title="Configure WordPress"
   ...
   ```php title="Find These Lines and Update Them with Your Database Details"
   ...
   **Option A only:** RDS requires SSL for all connections. Add this line immediately after `DB_HOST`:
   ...
   ```php title="Option a Only"
   define('MYSQL_CLIENT_FLAGS', MYSQLI_CLIENT_SSL);
   ```


   <Aside type="caution" title="Do Not Store Plaintext Credentials in Production">
...
1. **Create the VirtualHost configuration**
   ...
   ```bash title="Edit Apache VirtualHost"
   sudo vim /etc/apache2/sites-available/wordpress.conf
   ```

   Add the following content:

   ```apache title="Apache VirtualHost"
   <VirtualHost *:80>
...
2. **Enable the site and disable the default**
   ...
   ```bash title="Enable the Site and Disable the Default"
...
3. **Verify WordPress loads**
   ...
   ```bash title="Verify WordPress Loads"
...
1. **Create a health check script**
   ...
   ```bash title="Edit Health Check Script"
   sudo vim /usr/local/bin/wordpress-health.sh
   ```

   Add the following:

   ```bash title="wordpress-health.sh"
   #!/bin/bash
...
3. **Create a systemd timer unit**
   ...
   ```bash title="Create a Systemd Timer Unit"
   sudo vim /etc/systemd/system/wordpress-health.timer
   ```

   ```ini title="wordpress-health.timer"
   [Unit]
...
4. **Enable and start the timer**
   ...
   ```bash title="Enable and Start the Timer"
...
1. **Enable Apache to start on boot**
   ...
   ```bash title="Enable Apache to Start on Boot"
   sudo systemctl is-enabled apache2
   ```

   If it says `disabled`, enable it:

   ```bash title="Enable Service"
   sudo systemctl enable apache2
   ```

2. **Reboot the instance**

   ```bash title="Reboot the Instance"
   sudo reboot
   ```
...
3. **Verify everything survived**
   ...
   ```bash title="Verify Everything Survived"
   systemctl status apache2
   ```

   Check the health log for any gap during the reboot:

   ```bash title="Tail -5 /var/log/wordpress-health.log"
...
1. **Run nmap from your laptop**
   ...
   ```bash title="Run Nmap from Your Laptop"
...
<Aside type="danger" title="Terminate Lab Resources">
