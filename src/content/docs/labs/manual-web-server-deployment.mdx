...
A WordPress site requires three software components working together, often called the **LAMP stack**: **L**inux (the OS), **A**pache (the web server), **M**ariaDB (the database), and **P**HP (the programming language that WordPress is written in). You already have Linux. In this lab the database will run on a **separate server**, so on this instance you only install Apache and PHP.
