...
5. **Reconnect over SSH and verify the cluster**
   ...
   ```bash title="Reconnect over SSH and Verify the Cluster"
...
1. **Install Helm on the EC2 instance**
   ...
   ```bash title="Install Helm on the EC2 Instance"
...
2. **Install the monitoring stack**
   ...
   ```bash title="Install the Monitoring Stack"
   ...
   <Aside type="note" title="Helm Install Takes a Few Minutes">
...
3. **Wait for all Pods to be ready**
   ...
   ```bash title="Wait for All Pods to Be Ready"
...
4. **Access Grafana**
   ...
   ```text title="Deploying the Monitoring Stack Output"
   ...
   ```bash title="Retrieve Grafana Admin Password"
...
2. **Panel 1: Pod restart count**
   ...
   ```promql title="Pod Restart Count Query"
...
3. **Panel 2: Node memory usage percentage**
   ...
   ```promql title="Node Memory Usage Query"
...
4. **Panel 3: Pod ready status**
   ...
   ```promql title="Pod Ready Status Query"
...
1. **Create a PrometheusRule manifest**
   ...
   ```bash title="Edit alerting-rules.yaml"
...
2. **Apply the alert rules**
   ...
   ```bash title="Apply the Alert Rules"
...
3. **Verify in Prometheus**
   ...
   ```bash title="Verify in Prometheus"
...
1. **Trigger the crash loop alert**
   ...
   ```bash title="Trigger the Crash Loop Alert"
   for i in $(seq 1 5); do
...
4. **Investigate with logs**
   ...
   ```bash title="Investigate with Logs"
   kubectl get pods
   ```
...
```bash title="Remove Monitoring Resources"
