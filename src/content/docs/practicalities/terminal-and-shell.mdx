...
description: A practical reference for navigating the command line, understanding the shell, managing files and permissions, and working with pipes, redirection, and environment variables.
---
...
<Aside type="tip" title="Further Shell Practice">
...
<Aside type="caution" title="rm Is Permanent">
...
```bash title="Print Formatted Output"
printf "%-20s %s\n" "Directory" "Status"
printf "%-20s %s\n" "/var/www/html" "OK"
...
```bash title="Prompt for User Input"
...
```bash title="Compare Directories with Process Substitution"
...
```bash title="Switch Users with su"
...
```bash title="Reload Shell Configuration"
source ~/.bashrc   # bash
source ~/.zshrc    # zsh
```
...
```bash title="Open Help and Manual Pages"
