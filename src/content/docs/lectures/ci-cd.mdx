...
slug: ci-cd
order: 10
covers: deployment environments; CI vs continuous delivery vs continuous deployment; feedback loops and DORA metrics; pipeline design principles; CI/CD tools landscape; GitHub Actions concepts (workflows, events, jobs, steps, runners); triggers; marketplace actions and supply chain; secrets and OIDC; matrix builds; service containers; caching and artifacts; reusable workflows and composite actions; concurrency control; self-hosted runners; local testing with act; debugging; deployment strategies (rolling, blue-green, canary); preview environments and feature flags; operational concerns; merge queues on busy shared branches; GitHub merge_group
...
Because this single workflow handles pull request validation, merge-group validation, and `main`-branch delivery, the concurrency block conditionally cancels stale pull request runs while allowing `main` runs to finish. That keeps CI feedback tight without interrupting a deployment that is already in progress.

This workflow demonstrates the full feedback loop. A pull request or GitHub merge-group validation run triggers only the `quality` job (lint and test), giving fast feedback without the cost of a full build. A push to `main` runs `quality`, then `build-and-push`, then `deploy`, with the concurrency group ensuring that two rapid merges do not race each other into production. The `permissions` block at the top follows the principle of least privilege: the workflow can read repository contents and write packages, but nothing else.
...
- <a href="https://graydon2.dreamwidth.org/1597.html" target="_blank" rel="noopener noreferrer">The Not Rocket Science Rule of Software Engineering (Graydon Hoare)</a>
