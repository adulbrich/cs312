...
```yaml title="CI Pipeline Workflow"
name: CI Pipeline
...
```yaml title="Scheduled Workflow Trigger"
on:
  schedule:
...
```yaml title="Manual Workflow Trigger"
on:
  workflow_dispatch:
    inputs:
      environment:
...
```yaml title="CI/CD Pipeline Workflow"
name: CI/CD Pipeline
...
```yaml title="Build Image for Local Scan Step"
...
```yaml title="Deploy to Server Step"
...
```yaml title="Workflow Environment Variables"
env:
  NODE_ENV: production
...
```yaml title="Matrix Workflow Job"
jobs:
  test:
    runs-on: ${{ matrix.os }}
    strategy:
...
```yaml title="Cache Cargo Registry Step"
...
```yaml title="Upload Workflow Artifact"
...
```bash title="Install and Run act"
# Install
...
```yaml title="Upload Playwright Report Step"
...
```yaml title="Dependent Workflow Job"
jobs:
  deploy-staging:
    runs-on: ubuntu-latest
    environment: staging
...
```bash title="Blue-green Deployment"
...
```mermaid title="Merge Queues Diagram"
flowchart LR
...
```yaml title="Pull Request Trigger"
on:
  pull_request:
    branches: [main]
  merge_group:
