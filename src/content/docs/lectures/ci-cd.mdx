...
covers: deployment environments; CI vs continuous delivery vs continuous deployment; feedback loops and DORA metrics; pipeline design principles; CI/CD tools landscape; GitHub Actions concepts (workflows, events, jobs, steps, runners); triggers; marketplace actions and supply chain; secrets and OIDC; matrix builds; service containers; caching and artifacts; reusable workflows and composite actions; concurrency control; self-hosted runners; local testing with act; debugging; deployment strategies (rolling, blue-green, canary); preview environments and feature flags; operational concerns incl. container vulnerability scanning; merge queues on busy shared branches; GitHub merge_group
...
- <a href="https://trivy.dev/latest/docs/" target="_blank" rel="noopener noreferrer">Trivy Documentation (Aqua Security)</a>
