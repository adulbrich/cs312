...
**Single quotes vs. double quotes.** Double quotes allow variable expansion and command substitution; single quotes treat everything literally. `"Hello $NAME"` expands `$NAME`; `'Hello $NAME'` prints the literal string `$NAME`.
...
A backup script is only useful if it runs on a schedule. Cron is the traditional Unix job scheduler and is available on every Linux system. Each user has a crontab (cron table) that lists commands and their schedules. Edit yours with `crontab -e`. The format is five fields followed by the command:
