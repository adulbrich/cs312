...
```bash title="Set Source and Destination Variables"
#!/usr/bin/env bash
SOURCE="/var/www/html"
...
```bash title="Timestamped Backup Name"
...
```text title="Variables and Quoting Output"
...
```bash title="Argument Validation Script"
#!/usr/bin/env bash
if [[ "$#" -lt 2 ]]; then
...
```bash title="Sourcing Versus Executing"
...
```bash title="Exit Codes Are the Contract"
...
```bash title="Validate Script Preconditions"
#!/usr/bin/env bash
SOURCE="/var/www/html"
TIMESTAMP=$(date +%Y%m%d-%H%M%S)
DEST="/backups/html-backup-$TIMESTAMP"
...
```bash title="Root and Source Preconditions"
if [[ "$EUID" -ne 0 ]]; then
...
```bash title="Loop Over Source Directories"
#!/usr/bin/env bash
TIMESTAMP=$(date +%Y%m%d-%H%M%S)
...
```bash title="Arithmetic Counter Loop"
#!/usr/bin/env bash
i=0
while (( i < 5 )); do
    printf "i = %d\n" "$i"
...
```bash title="Reusable Logging Function"
#!/usr/bin/env bash
TIMESTAMP=$(date +%Y%m%d-%H%M%S)
...
```bash title="Trap Cleanup Pattern"
#!/usr/bin/env bash
set -euo pipefail
...
```bash title="Defensive Backup Script"
#!/usr/bin/env bash
set -euo pipefail
...
```bash title="Sed 'S/foo/bar/' Input.txt"
...
```bash title="Filesystem Usage with awk"
