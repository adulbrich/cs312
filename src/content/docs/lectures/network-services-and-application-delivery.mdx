...
order: 12
covers: DNS operations and troubleshooting; mail routing and authentication; firewalls and VPN/WAN patterns; reverse proxies and L4/L7 load balancing; CDNs; TLS automation and diagnostics
prereq_lectures: networking-fundamentals, container-orchestration
followup_lectures: monitoring-and-performance, log-management-and-analysis, system-security-and-hardening
paired_activities: network-services-exploration
...
```mermaid title="Nslookup Is Older but Ubiquitous, Including on Windows and"
sequenceDiagram
...
```mermaid title="Internal DNS and Split-horizon Diagram"
flowchart LR
...
```mermaid title="Placement and Zone Design Diagram"
flowchart TB
  Internet(["Internet"])
...
```bash title="List iptables Rules"
...
```mermaid title="Deployment Patterns Diagram"
flowchart LR
...
```mermaid title="Why Use a Reverse Proxy Diagram"
flowchart LR
  Client["Client"]
  RP["Reverse proxy\nTLS termination\nPath routing\nLoad balancing"]
  B1["Backend 1\n10.0.1.10:3000"]
...
```mermaid title="Layer 4 vs. Layer 7 Diagram"
flowchart LR
...
```mermaid title="Public and Internal Load Balancers in a VPC Diagram"
flowchart TB
  Internet(["Internet"])
...
```mermaid title="Content Delivery Networks Diagram"
flowchart LR
  User["User (Berlin)"]
...
```text title="HSTS (HTTP Strict Transport Security) Output"
...
```bash title="Inspect Certificate SANs"
...
```mermaid title="Takeaways Diagram"
flowchart TB
  User["User"]
  DNS["DNS resolver"]
  CDN["CDN edge PoP\nTLS termination, cache, WAF"]
