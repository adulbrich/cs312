...
order: 11
covers: orchestration problems and control loops; Kubernetes architecture; Pods, Deployments, Services, storage, probes, rollouts, and scaling; operators, GitOps, k3s, and managed Kubernetes
...
followup_lectures: network-services-and-application-delivery, monitoring-and-performance, log-management-and-analysis, system-security-and-hardening
paired_activities: minikube
...
```mermaid title="The Control Plane Diagram"
flowchart LR
...
```mermaid title="Worker Nodes Diagram"
flowchart LR
...
[**kube-proxy**](https://kubernetes.io/docs/reference/networking/virtual-ips/) usually runs on each node and maintains the network rules that make Services work. Some clusters let their networking layer handle that job instead, so kube-proxy is not always required.
...
```bash title="The Container Runtime"
...
```yaml title="postgres StatefulSet"
apiVersion: apps/v1
kind: StatefulSet
metadata:
  name: postgres
...
This example assumes a Service named `postgres` already exists. Setting `clusterIP: None` makes it a [**headless Service**](https://kubernetes.io/docs/concepts/services-networking/service/#headless-services), which means the Pods get their own DNS names instead of one shared ClusterIP.
...
```yaml title="nginx-service Service"
apiVersion: v1
kind: Service
metadata:
  name: nginx-service
...
```mermaid title="Namespaces Diagram"
flowchart TB
...
```bash title="Kubectl Create Namespace Production"
...
```mermaid title="Pod-to-pod Reachability Diagram"
flowchart LR
...
```mermaid title="Services and DNS Diagram"
flowchart LR
...
```yaml title="site-ingress Ingress"
...
```mermaid title="Reaching Services Beyond Cluster Internals Diagram"
flowchart TB
...
```yaml title="mysql-pvc PersistentVolumeClaim"
apiVersion: v1
...
```yaml title="api HorizontalPodAutoscaler"
...
For laptop and CI work, **[kind](https://kind.sigs.k8s.io/)** and **[minikube](https://minikube.sigs.k8s.io/)** are common choices. kind runs clusters as Docker containers, which makes it fast and disposable. minikube is more like a long-lived local cluster. For small servers, edge sites, and this course's orchestration work, **[k3s](https://k3s.io/)** is the most relevant lightweight distribution. It remains conformant Kubernetes, packages the control plane into a small binary, uses SQLite by default on a single-server install, and uses embedded etcd or an external datastore for High-Availability (HA). A default install bundles basics such as CoreDNS, metrics-server, local-path storage, and [Traefik](https://traefik.io/) as the Ingress controller.
...
For teams that want to assemble a self-managed cluster from first principles without a pre-packaged distribution, [**kubeadm**](https://kubernetes.io/docs/reference/setup-tools/kubeadm/) is the standard bootstrapping tool. It initializes the control plane on a first node, generates the required certificates and kubeconfig files, and produces join tokens that worker nodes use to register with the cluster. What kubeadm deliberately does not include is a CNI plugin, a storage provisioner, or an Ingress controller: those choices remain yours. Lightweight distributions like k3s bundle sensible defaults so you can have a working cluster in minutes; kubeadm gives you a correct cluster with no opinions, which suits environments where specific networking, storage, or compliance requirements make the bundled defaults unsuitable.
...
Plain manifests are the foundation, but teams rarely stop there. Once an application grows beyond a handful of YAML files, two higher-level patterns usually appear: packaging and overlaying. **[Helm](https://helm.sh/)** is the most common packaging tool in the Kubernetes ecosystem. A **chart** is a versioned package of Kubernetes objects, a **release** is one installed copy of that package in a cluster, and a **values** file supplies the inputs that customize it for a particular environment. This is why Helm shows up so often around shared infrastructure such as ingress controllers, cert-manager, and monitoring stacks: these systems involve many related objects that need to be installed, upgraded, and rolled back together.

**[Kustomize](https://kustomize.io/)** solves a nearby but different problem. Kustomize starts from manifests you already own and applies environment-specific patches to them. Helm is stronger when you want a reusable package with defaults and a configurable interface; Kustomize is stronger when you already understand the raw manifests and want a disciplined way to vary them between development, staging, and production. In practice, teams often use both, but for different ownership boundaries.

The main risk with Helm is abstraction. A chart can make installation easier while also hiding what is actually running if you treat it as a black box. A competent operator should still be able to answer basic questions about any chart they install: what objects did it create, which values are operationally important, what changes on upgrade, and how would you roll it back if the new release caused trouble.
...
- <a href="https://www.youtube.com/watch?v=BE77h7dmoQU" target="_blank" rel="noopener noreferrer">Kubernetes: The Documentary [PART 1] (YouTube)</a>
- <a href="https://www.youtube.com/watch?v=318elIq37PE" target="_blank" rel="noopener noreferrer">Kubernetes: The Documentary [PART 2] (YouTube)</a>
