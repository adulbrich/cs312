...
Key pairs are not the only access pattern you will encounter. [EC2 Instance Connect](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/connect-linux-inst-eic.html) can inject short-lived SSH keys on demand, authenticating the request through IAM rather than distributing a long-lived private key file. [AWS Systems Manager Session Manager](https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager.html) can provide shell access without opening inbound SSH at all: the SSM agent on the instance initiates an outbound WebSocket connection to the SSM service, so the security group requires no inbound rule on port 22. Windows instances use the key pair differently: the private key is used to decrypt the initial Administrator password rather than to populate `authorized_keys`.
...
For production infrastructure, the better pattern is to avoid managing long-lived key pairs for team access entirely. EC2 Instance Connect issues a fresh key pair per connection, valid for sixty seconds, and uses IAM to control who can request a connection. Systems Manager Session Manager does not use SSH at all and produces a complete audit log of every session in [AWS CloudTrail](https://docs.aws.amazon.com/awscloudtrail/latest/userguide/cloudtrail-user-guide.html) (the service that records every API call and management action taken in your AWS account), which is valuable for compliance and incident response. With either approach, the instance's security group can omit port 22 inbound entirely, which removes a common attack surface. For instances in private subnets, where there is no direct inbound path from the internet anyway, Systems Manager Session Manager is especially clean: the agent reaches out to AWS, no bastion host or VPN is required, and no special network routing is needed to administer the instance.
