...
For our small office, the layers map to concrete things. Layer 1 is the Cat6 cabling running from each desk to a patch panel in the server closet. Layer 2 is the Ethernet switch connecting those cables. Layer 3 is the router forwarding packets between subnets. DHCP assigns IP addresses, while the router handles packet forwarding. Layer 4 is the TCP connections carrying HTTP, SSH, and database traffic. Layer 7 is the web application itself. Every diagnostic step you take later in this chapter targets a specific layer.
...
For our office, the DHCP server (often running on the router or a dedicated Linux server) might be configured to hand out addresses from `10.0.1.10` through `10.0.1.126` in the workstation subnet, with the default gateway set to `10.0.1.1` and DNS pointed to `10.0.1.130` (the DNS server in the server subnet). Servers themselves typically use either **static addresses** or **DHCP reservations** so their addresses remain stable; clients and other services depend on those addresses being predictable.
...
dig wiki.office.example

# Shorter output showing just the answer
dig +short wiki.office.example

# Use nslookup for a quick interactive check
nslookup wiki.office.example
```
...
5. **Apply firewall rules.** Inbound traffic from the internet is blocked by default. Port forwarding is configured for any services that need external access. Between subnets, workstations can reach the web server (port 443) and DNS (port 53), but direct access to the database server is restricted to the web server only.
</Steps>

This design maps closely to cloud concepts. In AWS, the overall `10.0.1.0/24` block would be a VPC, each `/25` would be a subnet, the router is the VPC's implicit router, NAT is provided by a NAT Gateway, and firewall rules are expressed as security groups and network ACLs. Whether a subnet is public or private depends on its route table and gateway configuration, not on the CIDR block alone.
...
dig @10.0.1.130 wiki.office.example

# Reverse DNS lookup
dig -x 10.0.1.131

# Quick lookup with nslookup
nslookup wiki.office.example 10.0.1.130
```

The `dig` output includes the answer section (the actual records), the authority section (which name server is responsible), and timing information. If `dig @10.0.1.130` works but `dig` alone does not, the workstation is probably configured to use the wrong DNS server or is reaching a resolver with different data.

### curl: Testing Application-Layer Connectivity

`curl` makes HTTP (and many other protocol) requests from the command line. It is invaluable for testing whether a web service is actually responding. In this example, assume the office wiki is served over HTTPS on port 443:

```bash
# Test if the web server responds
curl -I https://wiki.office.example

# Test with verbose output to see TLS and header details
curl -v https://wiki.office.example

# Test the server by IP when DNS is suspect (-k ignores certificate-name mismatch)
curl -kI https://10.0.1.131

# Follow redirects and show timing
curl -o /dev/null -s -w "HTTP %{http_code} in %{time_total}s\n" https://wiki.office.example
```
...
3. `dig wiki.office.example` to verify DNS resolution
4. `curl -I https://wiki.office.example` to verify the web service is responding
...
6. `nmap -sV -Pn -p T:443 <public-ip>` (from outside the network) to confirm the port is reachable through any firewall or security group between you and the server.
</Aside>

## Putting It All Together

Networking can feel abstract until you connect the pieces. In our small office scenario, a single web request from an employee's laptop to `wiki.office.example` touches every concept in this chapter. The laptop uses DHCP to get an address, sends a DNS query over UDP port 53 to resolve the hostname, constructs an IP packet destined for `10.0.1.131`, determines via the routing table that this address is on a different subnet and sends the packet to the default gateway, the switch forwards the Ethernet frame to the router port, the router forwards it to the server subnet, the server accepts the HTTPS connection on port 443, and Nginx serves the wiki page. Understanding this chain, and knowing which tool to use at each step, is the core skill that separates confident troubleshooting from guesswork.
