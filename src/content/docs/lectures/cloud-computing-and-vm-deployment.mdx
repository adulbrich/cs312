---
title: Cloud Computing and VM Deployment
...
The components above correspond directly to the OSI layers covered in the networking fundamentals lecture:

| AWS Component | OSI Layer | Role |
|---------------|-----------|------|
| VPC | Layer 3 (Network) | Defines the IP address space (your virtual routing domain) |
...
| Network ACL | Layer 3–4 (Network / Transport) | Stateless per-subnet ACL; both inbound and outbound rules required |
| Network Load Balancer (NLB) | Layer 4 (Transport) | Distributes TCP/UDP connections without inspecting HTTP content |
| Application Load Balancer (ALB) | Layer 7 (Application) | Routes HTTP/HTTPS requests based on host headers, paths, and cookies |

Thinking in layers helps when troubleshooting. If a security group rule looks correct but traffic is still blocked, check the NACL; both operate at the same layer but at different scopes (instance vs. subnet). If packets are reaching the instance but the application returns errors, the problem is at Layer 7, above where security groups and NACLs operate.
...
S3 is ideal for backups, static assets, log archives, and data that does not require POSIX (Portable Operating System Interface, the standard that defines traditional Unix filesystem behaviors like file locking and directory traversal) filesystem semantics. Greenfield might use S3 to store nightly database backups, user-uploaded files, and application build artifacts.
