...
HTTPS requires a TLS certificate: a cryptographically signed document that proves the server owns the private key corresponding to the public key in the certificate, and that a trusted Certificate Authority (CA) has verified this. Without a valid certificate, browsers display security warnings and refuse to connect.

### How certificates work

A certificate contains the server's public key, the domain name(s) it is valid for, a validity period, and the CA's digital signature. The browser already trusts a set of root CAs (pre-installed by the OS or browser vendor). When a browser connects to a server, it verifies that the certificate's signature chains up to one of those trusted roots and that the certificate is not expired or revoked.

### Let's Encrypt and automated certificate management

Before 2016, obtaining a certificate required purchasing one from a commercial CA and manually renewing it every one to two years. Let's Encrypt changed this by providing free certificates via an automated protocol called ACME (Automated Certificate Management Environment). The dominant ACME client is **certbot**:

```bash
# Install certbot on Ubuntu
sudo apt install certbot python3-certbot-apache

# Obtain and install a certificate for a domain (Apache)
...
Certbot contacts Let's Encrypt, proves domain ownership (by temporarily serving a challenge file on port 80 or by adding a DNS TXT record), receives a certificate valid for 90 days, and installs it. A systemd timer runs `certbot renew` twice daily; certificates are renewed automatically when they have less than 30 days remaining.
...
<Aside type="caution">
Never use a wildcard certificate (`*.company.com`) for all services. If the private key is compromised, every service using that certificate is vulnerable. Issue per-service certificates and automate renewal so the overhead of short-lived certificates is manageable.
</Aside>
