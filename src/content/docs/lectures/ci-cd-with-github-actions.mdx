...
| Acceptance | UAT (User Acceptance Testing) | Business stakeholders verify requirements are met |
...
<HistoricalNote title="From CruiseControl to GitHub Actions (2001-2018)">
The phrase "continuous integration" appeared in Grady Booch's 1994 object-oriented design book, though the practice he described was far less automated than modern CI. Kent Beck's Extreme Programming (1999) formalized it: merge small changes frequently, run tests immediately, fix failures before continuing. CruiseControl (2001) was the first open-source CI server. Jenkins (2011, a fork of the older Hudson project) became ubiquitous because it was free, self-hosted, and infinitely extensible with plugins. Travis CI (2011) changed the economics for open-source projects by offering hosted CI integrated directly with GitHub, with no server to maintain. GitHub Actions launched in 2018, bringing CI/CD infrastructure into the same interface where code already lived and eliminating the operational overhead of running a separate CI system. Most new projects today use a hosted service rather than managing their own CI infrastructure, a complete reversal from the Jenkins era.
</HistoricalNote>
...
**Canary deployment** sends a small fraction of real production traffic (say, 5%) to the new version while the remaining 95% continues hitting the old version. The name comes from the mining practice of carrying a canary into a coal mine: if the canary dies, something is wrong. You monitor error rates, latency, and business metrics on the canary. If everything looks healthy after a set period, you gradually shift more traffic over. If the metrics degrade, you pull all traffic back to the old version before most users were ever affected. Canary releases are more complex to implement (they require a traffic-splitting layer such as a load balancer with weighted routing or a service mesh, a dedicated infrastructure layer that manages traffic between microservices) but give you real-world validation with limited blast radius.

| Pattern | Downtime | Rollback speed | Infrastructure cost | Version coexistence |
|---------|----------|----------------|---------------------|---------------------|
| Rolling | None | Minutes (re-roll forward/back) | None | Yes, briefly |
| Blue-green | None | Instant (traffic switch) | 2x production | No |
| Canary | None | Fast (redirect traffic) | Moderate | Yes, intentionally |

In practice, these patterns combine with the pipeline mechanics above. A typical setup uses a rolling or canary deployment to staging on every merge to `main`, then requires manual approval and performs a blue-green swap to production.
