...
followup_lectures: incident-response-and-postmortems
paired_activities: follow-the-logs
...
```bash title="Logging on macOS and Windows"
...
```bash title="List Hardware Event Logs"
...
```bash title="Stream nginx 500 Errors"
...
```bash title="Count Status Codes with awk"
...
```bash title="Count Requests by Minute"
...
```bash title="Count 5xx Errors by Minute"
...
```text title="logrotate Sampling Configuration"
/var/log/nginx/*.log {
    daily
    missingok
    rotate 14
...
```text title="Loki Connection Refused Query"
...
```text title="Loki Error Duration Query"
...
```text title="Fatal Log Alert Query"
...
- <a href="https://www.youtube.com/watch?v=KK9FI4OfPUY" target="_blank" rel="noopener noreferrer">Central Log Management for Docker + Linux // Grafana Loki (Christian Lampa) (YouTube)</a>
...
- <a href="https://slack.engineering/streamlining-security-investigations-with-agents/" target="_blank" rel="noopener noreferrer">Streamlining Security Investigations with Agents (Slack Engineering)</a>