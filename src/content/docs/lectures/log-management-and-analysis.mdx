...
The **severity level** classifies the importance of the event. The [syslog standard](https://datatracker.ietf.org/doc/html/rfc5424#section-6.2.1) defines eight levels numbered 0 through 7, from EMERG (the system is unusable) at the top to DEBUG (fine-grained detail intended for development use) at the bottom. Most application frameworks use a simplified subset: DEBUG, INFO, WARN, ERROR, and FATAL or CRITICAL. In production, most services should log at INFO or WARN and above. Running at DEBUG in production generates enormous volume, can expose sensitive data in log output, and obscures the signals you need when investigating an incident.
...
**Full credit card numbers, social security numbers, and other regulated PII (Personally Identifiable Information).** <a href="https://www.pcisecuritystandards.org/" target="_blank" rel="noopener noreferrer">PCI-DSS</a> (Payment Card Industry Data Security Standard), <a href="https://www.hhs.gov/hipaa/" target="_blank" rel="noopener noreferrer">HIPAA</a> (Health Insurance Portability and Accountability Act), <a href="https://gdpr-info.eu/" target="_blank" rel="noopener noreferrer">GDPR</a> (General Data Protection Regulation), and most other compliance frameworks explicitly prohibit logging regulated identifiers in plaintext. Logging masked or tokenized versions (last four digits of a card number, a hash of a Social Security Number) is generally acceptable; logging the full value is not.
...
<Aside type="note" title="Logging on macOS and Windows">
On macOS, the primary system log mechanism is the **Unified Logging System (ULS)**, which replaced syslog. Logs are stored in `/var/log/` for some older daemons, but system logs are primarily queried via the `log` command: `log show --predicate 'eventMessage contains[c] "error"' --info --last 1h`. Windows uses the **Event Log**, managed by the Event Viewer GUI or queried via PowerShell: `Get-EventLog -LogName Application -EntryType Error -Newest 100`. Windows also writes application logs to files under `C:\ProgramData\` or application-specific directories. The underlying principle is the same across all platforms: applications write structured or unstructured events to a system log store, operators retrieve and search those events, and centralized collection agents aggregate logs from many machines. The tools and paths differ, but the investigation workflow is portable.
</Aside>
...
The audit log at `/var/log/audit/audit.log` records structured entries for each matched event, including the timestamp, the user who triggered it, the process name and PID, and the specific system call and arguments. This is the log that compliance auditors typically request: PCI-DSS requires audit trails for access to cardholder data systems, HIPAA requires audit logs for access to protected health information, and <a href="https://www.fedramp.gov/" target="_blank" rel="noopener noreferrer">FedRAMP</a> (Federal Risk and Authorization Management Program) requires system-level audit logging with specific retention periods.
...
# grep -oP 'for \K\S+': print only the username token after "for"
```
...
On Linux, [`logrotate`](https://www.man7.org/linux/man-pages/man8/logrotate.8.html) is the standard tool. It runs daily via a systemd timer or cron job and reads configuration from `/etc/logrotate.conf` (global defaults) and `/etc/logrotate.d/` (per-application overrides). A typical configuration for Nginx:
...
[**osquery**](https://www.osquery.io/) is an open-source tool that exposes operating system state as SQL tables. You can query for processes running from deleted binaries (a common malware indicator), unusual network connections, and suspicious file modifications. In practice, osquery is most useful when deployed continuously and shipping scheduled query results to a central store, rather than run manually only during incidents.
...
- <a href="https://www.youtube.com/watch?v=KK9FI4OfPUY" target="_blank" rel="noopener noreferrer">Central Log Management for Docker + Linux // Grafana Loki (Chistian Lampa) (YouTube)</a>
- <a href="https://www.youtube.com/watch?v=B2IS-XS-cc0" target="_blank" rel="noopener noreferrer">Fluent Bit explained | Fluent Bit vs Fluentd (TechWorld with Nana) (YouTube)</a>
- <a href="https://stripe.com/blog/canonical-log-lines" target="_blank" rel="noopener noreferrer">Fast and flexible observability with canonical log lines (Brandur Leach) (Stripe)</a>
- <a href="https://sre.google/sre-book/monitoring-distributed-systems/" target="_blank" rel="noopener noreferrer">Monitoring Distributed Systems (Rob Ewaschuk) (Google SRE Book)</a>
- <a href="https://www.youtube.com/watch?v=WSW1urIXsfA" target="_blank" rel="noopener noreferrer">Open Source Observability Explained - The Grafana Stack (Grafana) (YouTube)</a>
