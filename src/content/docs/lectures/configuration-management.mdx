...
This lecture covers **Ansible**, one of the most widely adopted configuration management tools: how its agentless architecture enables management at scale, how inventories and playbooks express the desired state of a fleet, how idempotency makes playbooks safe to run repeatedly, and how roles and templates organize a codebase that can manage hundreds of servers from a single command. If the Shell Scripting lecture made the case for Bash as the right tool for single-machine automation, this lecture explains what takes over when the fleet grows beyond what any script can reliably manage.
...
As covered in the Infrastructure as Code lecture, the declarative versus imperative distinction applies equally to configuration management. A procedural tool follows a sequence of steps in order, the same way a Bash script does. A declarative tool inspects the current state of each server, determines what changes are necessary to reach the declared goal, and applies only those changes. Running the same procedural script twice can cause problems: it may install something already installed, or restart a service that is already running correctly. Running the same declarative playbook twice is safe: if a server already matches the declared state, the playbook reports "ok" and touches nothing.

Ansible uses the declarative model. Rather than writing "run apt-get install nginx," you declare "nginx should be in the installed state." If nginx is already installed, Ansible does nothing. This property is called **idempotency**, and it is central enough to Ansible's design that it gets its own section in this lecture.
...
<Aside type="tip" title="Default Inventory Groups">
...
<Aside type="caution" title="When Idempotency Is Not Automatic">
...
<Aside type="caution" title="Check Mode Is Not a Full Simulation">
...
Configuration management occupies a specific position in the infrastructure automation stack. Terraform, as covered in the Infrastructure as Code lecture, provisions infrastructure objects: it creates EC2 instances, networks, and databases. Configuration management picks up from there, operating on already-running servers to install software, deploy configuration files, and ensure services are in the correct state. CI/CD pipelines, covered in the CI/CD Pipelines and GitHub Actions lecture, then automate the delivery of application code onto that configured infrastructure. Each tool hands off cleanly to the next, and each operates at a different level of abstraction.

Within Ansible's own design, the layers build on each other logically. The inventory identifies the fleet and organizes hosts into groups. Playbooks target those groups and execute plays. Each play invokes tasks, each task calls a module, and each module guarantees idempotency for its domain: the `apt` module will not install a package that is already installed; the `copy` module will not overwrite a file whose checksum already matches; the `service` module will not restart a service that is already running in the desired state. The result is that running the same playbook against the same fleet repeatedly produces the same outcome, which is the property that makes automation trustworthy rather than anxious.

Scaling from three servers to three hundred requires no architectural changes: a longer inventory file and faster parallel SSH connections. When a new server joins the fleet, adding it to the inventory and running the playbook is sufficient: the server converges toward the declared state. When a configuration change must reach every web server, you change one template, commit it, and run one command. The difference between managing a small fleet and a large one is the length of the inventory file, not the amount of manual work.

The discipline that keeps this reliable is treating Ansible code with the same rigor as application code. Playbooks belong in version control. Changes should be reviewed before they reach production, tested in a staging environment first, and organized into roles that are reusable across projects. A playbook that lives only on one engineer's laptop is infrastructure that can only be changed by that engineer, which is its own form of configuration drift.
...
- <a href="https://docs.ansible.com/ansible/latest/getting_started/index.html" target="_blank" rel="noopener noreferrer">Getting Started with Ansible (Ansible Documentation)</a>
- <a href="https://docs.ansible.com/ansible/latest/playbook_guide/index.html" target="_blank" rel="noopener noreferrer">Ansible Playbook Guide (Ansible Documentation)</a>
- <a href="https://docs.ansible.com/ansible/latest/user_guide/vault.html" target="_blank" rel="noopener noreferrer">Ansible Vault: Protecting Sensitive Data (Ansible Documentation)</a>
- <a href="https://galaxy.ansible.com/" target="_blank" rel="noopener noreferrer">Ansible Galaxy: Community Collections and Roles Hub (galaxy.ansible.com)</a>
- <a href="https://www.ansiblefordevops.com/" target="_blank" rel="noopener noreferrer">Ansible for DevOps (Jeff Geerling) (Book)</a>
- <a href="https://www.youtube.com/watch?v=xRMPKQweySE" target="_blank" rel="noopener noreferrer">Getting started with Ansible (Jeff Geerling) (YouTube)</a>
