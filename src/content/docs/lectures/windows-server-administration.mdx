...
order: 19
covers: Windows Server 2025 context; AD DS/IdP model; domains, forests, sites, and trusts; Kerberos, LDAP, DNS; OUs, groups, GPOs; SMB/NTFS and cloud parallels; hybrid identity with Entra ID
...
```mermaid title="The Whole System at a Glance Diagram"
flowchart TB
...
```mermaid title="Domains, Forests, Sites, and Trusts Diagram"
flowchart TB
...
```mermaid title="Kerberos, LDAP, and the Shadow of NTLM Diagram"
sequenceDiagram
...
```mermaid title="Processing Order, Inheritance, and Precedence Diagram"
flowchart LR
...
```powershell title="PowerShell Command"
gpresult /r
...
[`gpresult /r`](https://learn.microsoft.com/en-us/windows-server/administration/windows-commands/gpresult) is usually the starting point. [`Get-GPResultantSetOfPolicy`](https://learn.microsoft.com/en-us/powershell/module/grouppolicy/get-gpresultantsetofpolicy) is optional depth, not a mandatory second step every time.
...
## Identity in a Hybrid or Cloud-Only World
...
### A Cloud-Only Company Can Be Real
...
| Classic Windows Problem | Cloud-First Microsoft Answer | What Stays the Same |
|---|---|---|
| Central sign-in for users and machines | Microsoft Entra ID plus Microsoft Entra join | One authoritative identity system decides who may authenticate |
| Push device settings and restrictions | Intune configuration and compliance policies | Centrally scoped policy still needs good targeting and change control |
| Shared team files on SMB shares | SharePoint Online, Teams files, and OneDrive | Group-oriented access to shared content still needs clear ownership |
| Kerberos and LDAP for app trust | OpenID Connect, OAuth 2.0, and SAML | Applications still defer to a central identity provider |
| Stable UNC paths and server abstractions | Stable SharePoint sites, libraries, and Teams spaces | Users still need stable names even when back-end implementations change |
...
- <a href="https://www.youtube.com/watch?v=MUDjd44t2GE" target="_blank" rel="noopener">Installing a Windows Server in my Homelab, am I crazy? (Christian Lampa) (YouTube)</a>
- <a href="https://www.youtube.com/watch?v=ehKkPRR6PmI" target="_blank" rel="noopener">Windows Active Directory, how it works? Users, Permissions, Policies (Christian Lampa) (YouTube)</a>
