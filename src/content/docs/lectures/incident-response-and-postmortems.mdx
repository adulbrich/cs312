...
order: 15
covers: incident definition and severity tied to SLOs; lifecycle as posture shifts (detect, triage, mitigate, resolve, learn); mitigation principles (reversibility, blast radius, time-to-undo); rollback, feature flags, traffic shifting, scaling, graceful degradation, Kubernetes-native primitives; roles (IC, ops lead, comms lead, scribe) and the IC paradox; communication discipline (internal channel, status page, bridge calls, audience hierarchy); decision-making under uncertainty (System 1/2, hindsight bias, local rationality, CRM); recovery and verification; blameless postmortems (Just Culture, six-part structure, 5 Whys, action items, the three-week test)
...
The most important property of a severity scheme is that the thresholds map to measurable signals, not to gut feelings. If a service has a 99.9 percent availability SLO, the Monitoring lecture's framing applies directly: an error rate sustained above 0.1 percent is, by definition, burning the error budget faster than it can replenish. That is a SEV-1 trigger that does not require debate at 2 AM. Tying severity to SLOs means the question "is this serious enough to wake someone up?" has already been answered, in writing, before anyone is on call.
...
**Resolve** is the verifying posture. Mitigation reduces user impact; resolution returns the system to a state that does not depend on emergency intervention. A service running on its rollback version because the new release crashed is mitigated, not resolved. The transition from mitigate to resolve is the moment the responders allow themselves to slow down: the immediate crisis is past, and the work shifts from "as fast as possible" to "as correctly as possible."
...
The Scribe role is the one most commonly skipped, and skipping it is the most common cause of poor postmortems. The engineer doing the technical work cannot reliably also document the timeline; their attention is on the system, not the record. If a team can staff only two roles, IC and Scribe are the right two. A coordinated response with a thin technical bench can still produce a good postmortem if the timeline is captured well, while a brilliant technical response without a timeline becomes hard to learn from.

### Small Teams

On teams of three or four engineers, one person often fills multiple roles. That is workable, but the roles should still be declared explicitly. "I am acting as IC and Comms for this one" is more useful than implicit role assumption, because it tells everyone else where to direct decisions and communications. The role that combines least well with technical work is Scribe, for the reason given above. When a small team must compress, prefer combining IC and Comms or folding Comms into the IC's responsibilities over asking the Operations Lead to also keep the timeline.
...
Crew Resource Management originated at a 1979 NASA workshop on cockpit communication and was widely adopted across commercial aviation through the 1980s. Its effects on safety were substantial. The accident rate per million flight hours fell dramatically over the following decades, and post-incident reviews increasingly identified communication failures rather than pure technical errors as the primary cause of accidents that did occur. The software industry began borrowing CRM concepts in the 2000s, most visibly in Google's SRE practice and Netflix's chaos engineering culture. The Google SRE book's chapter on incident response explicitly cites CRM as an inspiration. The convergence is not coincidental: both industries operate complex systems where consequences are large, response time is short, and the cognitive load of operating the system reliably exceeds what any one person can manage alone.
</HistoricalNote>

### Time-Boxing

A useful IC habit is to time-box investigations. "Spend the next ten minutes confirming whether this is a database problem. If you do not have an answer by 14:25, we move to traffic shifting away from the affected zone regardless." Time-boxing has two effects. It bounds the cost of a hypothesis that turns out to be wrong, and it forces a deliberate reconsideration at the boundary, where the responder is asked to step back and re-evaluate rather than continuing to push on the same investigation through sunk cost. ICs who use time-boxing produce shorter incidents, on average, than ICs who do not.
...
The aviation industry built its safety record on this principle. The Aviation Safety Reporting System, administered by NASA since 1976, accepts confidential reports of safety incidents from pilots, controllers, and crew. Reports are de-identified and analyzed for systemic patterns. Crucially, filing a report grants immunity from FAA enforcement action for non-deliberate violations disclosed in the report. The system works because the people closest to the failures have a credible reason to report them honestly. The software industry's adoption of blameless postmortems is the same idea applied to the same kind of complex socio-technical system.
...
**Root cause analysis**, often done with the 5 Whys technique. The 5 Whys is simple in concept and hard to do well. You ask "why did this happen?" of the symptom, then "why?" of the answer, and so on, until you reach a cause that is structural rather than proximate. A 5 Whys analysis that ends at "the engineer made a mistake" is incomplete. It should continue until it reaches "the system allowed this kind of mistake to cause this kind of damage," which is where the actionable change lives.
...
The five-phase lifecycle (detect, triage, mitigate, resolve, learn) is a tool for noticing what kind of posture the response is currently in and changing it deliberately. The mitigation toolkit (rollback, feature flags, traffic shifting, scaling, graceful degradation) is governed by three principles: reversibility, blast radius, and time-to-undo. The roles (IC, Ops Lead, Comms Lead, Scribe) exist to separate the cognitive functions of the response so that no one person is asked to do all of them. Communication is part of the response, not a separate concern: how the rest of the organization perceives the incident shapes how much downstream work the response will cost.

The cognitive layer matters as much as the procedural one. Hindsight bias and local rationality shape both the response and the postmortem; the cultural conditions for honest reflection must be designed for, not assumed. The aviation industry's adoption of Crew Resource Management offers a four-decade case study in what it looks like to take human factors seriously in a complex operational domain. Blameless postmortems are not a generosity but a structural precondition for learning, and their value lives entirely in whether the action items they produce actually get completed.
...
- <a href="https://increment.com/on-call/" target="_blank" rel="noopener noreferrer">On Call (Increment Magazine, Issue 5)</a>
