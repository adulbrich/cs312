...
```mermaid title="Server Cpus and NUMA Diagram"
flowchart TB
...
```mermaid title="Out-of-band Management Diagram"
flowchart LR
...
```mermaid title="Direct-attached, Network-attached, and Storage Area Networks"
flowchart LR
...
```mermaid title="From Three-tier to Spine-leaf Diagram"
flowchart TD
...
```mermaid title="VLAN Operations in Practice Diagram"
flowchart LR
  Server["Server\nuntagged VLAN 10"]
  PC["Workstation\nuntagged VLAN 20"]
  SW1["Switch A"]
...
```mermaid title="Power: from Utility to Server Diagram"
flowchart LR
  Utility["Utility\ngrid"]
  Gen["Diesel /\ngas generator"]
  ATS["ATS\nautomatic\ntransfer switch"]
