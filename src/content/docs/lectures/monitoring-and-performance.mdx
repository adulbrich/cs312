...
```mermaid title="Prometheus: Architecture and the Pull Model"
flowchart LR
...
```text title="Prometheus: Architecture and the Pull Model Output"
...
```promql title="HTTP Request Rate by Job"
...
```promql title="Restart Increase Query"
...
```promql title="Histogramquantile() Query"
...
```promql title="Aggregation Query"
...
```yaml title="Recording Rule Example"
groups:
  - name: latency.rules
    rules:
...
```yaml title="Alert Fatigue Is a Safety Problem"
groups:
  - name: watchdog
    rules:
      - alert: Watchdog
...
```promql title="Burn Rate Against 99.9% SLO"
(
