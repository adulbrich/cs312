...
<HistoricalNote title="Blameless Postmortems Came from Aviation (1976-2009)">
The modern engineering postmortem was largely popularized by John Allspaw and Paul Hammond's 2009 Velocity Conference talk, "10+ Deploys per Day: Dev and Ops Cooperation at Flickr." They argued that shipping frequently requires a culture where engineers can report mistakes honestly, which requires removing blame from incident reviews. Etsy's engineering blog documented their application of this philosophy throughout the early 2010s, and Google codified it in their 2016 Site Reliability Engineering book. The underlying idea is older: the aviation industry's Aviation Safety Reporting System (ASRS), created in 1976 by NASA, used confidential no-penalty incident reporting so that pilots and air traffic controllers could report close calls without fear. Aviation's safety record (the safest form of mass transportation) is partly a product of that reporting culture. Software engineering borrowed the same framework about 30 years later.
</HistoricalNote>
...
**Missing timestamps.** "At some point the DBA (Database Administrator) was paged" is not useful. If the exact time is unknown, note that explicitly: "~09:51 (approximate)."
...
Before an incident happens, establish who needs to know what, through which channel, and how often. The table below uses SEV-1 (Severity 1, the highest severity level, indicating complete service disruption) as the example cadence; lower-severity incidents may require less frequent updates.
