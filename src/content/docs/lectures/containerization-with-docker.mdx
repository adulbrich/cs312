...
- **Volume drivers:** the volume abstraction is pluggable. Drivers exist for NFS, AWS EBS, GCS, and other remote storage backends, so a volume can follow a container across hosts in an orchestrated environment rather than being tied to local disk.
</Aside>
...
docker compose pull myusername/my-app:1.2.3
docker compose up -d
```
