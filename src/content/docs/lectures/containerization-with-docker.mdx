...
```mermaid title="Docker Architecture Diagram"
flowchart TB
  CLI["docker\nCLI client"]
  daemon["dockerd\nDocker daemon"]
...
```mermaid title="Images and Layers Diagram"
flowchart TB
...
```bash title="Restart Unless-stopped"
...
```mermaid title="Bridge Networks Diagram"
flowchart LR
  subgraph host["Host"]
    h1[":8080"]
    h2[":9090"]
...
```bash title="Docker Network Create Myapp"
...
```bash title="Tag and Push Container Images"
# Tag for Docker Hub
...
```bash title="Docker Compose Pull Web"
...
```text title="Docker Swarm Output"
.git
.venv
__pycache__
*.pyc
