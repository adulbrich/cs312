...
Running a company network requires two complementary skills: designing and building it correctly, then diagnosing it when something breaks. This lecture covers both.

The scenario running through this lecture is a small company with twenty employees, servers in a closet, a handful of remote workers, a cloud presence, and a public website. You will learn how to design the logical structure of that network (VLANs, DHCP, DNS, routing), configure the services that make it usable (firewalls, reverse proxies, TLS certificates, email, VPNs), and connect multiple locations together. Along the way, the same scenario grounds every topic in a concrete troubleshooting problem.

The Monday-morning alert: "The website is down." Users cannot reach the company's internal web application. The application server is running, the database is healthy, and nothing changed over the weekend. Somewhere between the users and the server, something is broken. A sysadmin who built the network knows exactly where to look. This lecture develops that knowledge.
...
**PTR records** provide reverse DNS: given an IP address, they return a hostname. Reverse lookups use a special zone in the format `<reversed-octets>.in-addr.arpa`. For example, the PTR record for `203.0.113.50` lives at `50.113.0.203.in-addr.arpa`. ISPs manage PTR records for the IP blocks they allocate; end organizations can only set PTR records if they own the IP block outright or request their ISP to set them. PTR records matter for two reasons: many mail servers reject email from IPs without a matching PTR record, and `traceroute` uses reverse DNS to display router hostnames alongside IP addresses.

**SOA (Start of Authority) records** appear at the apex of every DNS zone. They specify the primary nameserver for the zone, an administrative contact email (written with a dot instead of `@`), and five timing parameters that control how secondary nameservers cache and expire zone data. You encounter SOA records when a PTR lookup returns NXDOMAIN: the SOA in the authority section of the response names the organization responsible for that IP block, which tells you whose abuse contact to reach and confirms the lookup reached the authoritative source.
...
**Cloud connectivity**: when one of your "sites" is an AWS VPC or Azure VNet, you have two options. A **VPN Gateway** creates an IPsec tunnel from your office router to the cloud VPC -- inexpensive but limited bandwidth and latency. **AWS Direct Connect** or **Azure ExpressRoute** are dedicated private fiber connections from your office to the cloud provider's colocation facility -- higher cost, predictable latency, and no traffic on the public internet.
