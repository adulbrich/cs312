...
`traceroute` reveals the path packets take to a destination by exploiting the IP TTL (Time To Live) field. It sends a series of packets with progressively increasing TTLs: first TTL=1 (the first router decrements it to zero, drops the packet, and sends back an ICMP "time exceeded" message), then TTL=2 (reaches the second hop), and so on. The round-trip times from each ICMP response let you see both the path and the latency at each hop.
