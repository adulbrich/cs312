...
```mermaid title="Tree (Hierarchical) Diagram"
flowchart TB
...
```mermaid title="DHCP (Dynamic Host Configuration Protocol) Diagram"
sequenceDiagram
...
```bash title="DNS Operates over Two Different Protocols"
...
```bash title="ICE (Interactive Connectivity Establishment)"
...
```text title="Show the Routing Table Output"
...
```mermaid title="Ports and Protocols Diagram"
sequenceDiagram
    participant C as Client
...
```text title="Show All Established Connections Output"
...
```yaml title="Netplan Static Address Example"
network:
  version: 2
  ethernets:
    enp3s0:
...
```text title="Use Tcp-based Traceroute on a Specific Port (Useful When"
...
```bash title="Query DNS with dig and nslookup"
...
```bash title="Nmap -sv -pn -p T:22 10.0.1.10"
...
```text title="Check a Non-standard Application Port (E.g., a Minecraft"
...
```bash title="Tcpdump -i Eth0 Host 10.0.1.50"
