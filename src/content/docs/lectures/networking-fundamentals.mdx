...
order: 4
covers: OSI/TCP-IP models; IP addressing/subnetting (CIDR); DHCP; DNS records; NAT; TLS basics; routing tables and default gateway; ARP mechanics; ports and protocols; Linux IP configuration; diagnostic tools (ping, traceroute, dig, nslookup, curl, openssl s_client, nmap, tcpdump); bottom-up debugging workflow
prereq_lectures:
followup_lectures: network-services-and-application-delivery, on-premises-infrastructure
