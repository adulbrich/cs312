...
2. **Narrated screen recording (max 3 minutes)**. Your server MOTD must include your name or student ID. **Submit timestamps alongside the video** (e.g., "Checkpoint 1: 0:00, Checkpoint 2: 0:38, ..."):
   1. Show the running container (`docker ps` or equivalent), run `nmap -sV -Pn -p T:25565 <public-ip>` to confirm reachability and display your custom MOTD, then reboot the host and confirm the container returns automatically.
   2. Place a marker file in the world directory (`touch world/PERSISTENCE_TEST`) and note the modification time of `level.dat`, stop and remove the container, recreate it from the same image, and show the marker file still exists with `level.dat` timestamp unchanged.
   3. Upgrade to a newer pinned ECR image version and confirm the service is healthy (e.g., `nmap` or container status), then roll back to the prior known-good version and confirm it runs correctly.
