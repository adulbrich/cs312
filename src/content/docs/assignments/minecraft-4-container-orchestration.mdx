...
import rubricTsv from '../../../../canvas/assignments/assignment-4-rubrics.tsv?raw'
...
- k3s uses `containerd` as its container runtime. Your ECR images push and pull normally, but k3s still needs an authentication path for private ECR pulls. On EC2, prefer a [node-level approach](https://kubernetes.io/docs/tasks/administer-cluster/kubelet-credential-provider/) that uses the attached IAM instance profile to obtain short-lived ECR credentials at pull time. If you instead use a Kubernetes `imagePullSecret`, treat it as a fallback and document how the temporary ECR token is created and refreshed.
...
2. **Narrated screen recording (max 3 minutes)**. Your server MOTD must include your name or student ID. Submit timestamps alongside the video (e.g., "Checkpoint 1: 0:00, Checkpoint 2: 0:38, ..."):
  ...
  2. `kubectl delete pod <minecraft-pod>` followed by `kubectl get pods` showing the replacement pod come up, then evidence that the replacement pod mounts the same PVC and that the world directory still contains your data. If you need a concrete verification pattern, adapt the persistence-check approach from [Ops 2, section B](/assignments/minecraft-2-containerized-server/#b-state-boundary-and-persistence).
