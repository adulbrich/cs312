...
1. **PDF** containing: architecture diagram, runbook updates (persistence, backup/restore, rollout/rollback), failure drill report, and a link to your GitHub repository with Terraform and Kubernetes manifest changes.
2. **Narrated screen recording (max 3 minutes)**. Your server MOTD must include your name or student ID. Submit timestamps alongside the video (e.g., "Checkpoint 1: 0:00, Checkpoint 2: 0:38, ..."):
  1. `kubectl get nodes/pods` showing Minecraft running, then `nmap -sV -Pn -p T:25565 <public-endpoint>` showing 25565/tcp reachable and displaying your custom MOTD.
