...
- Your MOTD (displayed to connecting clients and by `nmap`) is set via the `motd=` line in `server.properties`.
...
2. **Narrated screen recording (max 3 minutes)**. Your server MOTD must include your name or student ID. **Submit timestamps alongside the video** (e.g., "Checkpoint 1: 0:00, Checkpoint 2: 0:38, ..."):
   ...
   2. Reboot the instance, then run `nmap -sV -Pn -p T:25565 <public-ip>` showing the service is reachable and your custom MOTD (port can differ if you choose a non-default Minecraft server configuration).
